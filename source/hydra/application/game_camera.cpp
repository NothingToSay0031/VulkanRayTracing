...
// GameCamera //////////////////////////////////////////////////////////////////
void GameCamera::init(bool enabled_, f32 rotation_speed_, f32 movement_speed_,
                      f32 movement_delta_) {
  reset();
  enabled = enabled_;
...
void GameCamera::update(InputService* input, u32 window_width, u32 window_height, f32 delta_time) {
  ...
  if (input->is_mouse_dragging(MOUSE_BUTTONS_RIGHT) && !ImGui::IsAnyItemHovered()) {
    if (ignore_dragging_frames == 0) {
      target_yaw -=
          (input->mouse_position.x - roundu32(window_width / 2.f)) * mouse_sensitivity * delta_time;
      target_pitch -= (input->mouse_position.y - roundu32(window_height / 2.f)) *
                      mouse_sensitivity * delta_time;
    } else {
  ...
  if (input->is_key_down(KEY_LEFT) || input->is_key_down(KEY_A)) {
    camera_movement =
        glms_vec3_add(camera_movement, glms_vec3_scale(camera.right, -camera_movement_delta));
  ...
  if (input->is_key_down(KEY_PAGEDOWN) || input->is_key_down(KEY_E)) {
    camera_movement =
        glms_vec3_add(camera_movement, glms_vec3_scale(camera.up, -camera_movement_delta));
  ...
  if (input->is_key_down(KEY_UP) || input->is_key_down(KEY_W)) {
    camera_movement =
        glms_vec3_add(camera_movement, glms_vec3_scale(camera.direction, camera_movement_delta));
