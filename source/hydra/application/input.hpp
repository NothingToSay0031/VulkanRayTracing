...
//
//
enum Device : u8 { DEVICE_KEYBOARD, DEVICE_MOUSE, DEVICE_GAMEPAD };  // enum Device
...
struct Gamepad {
  ...
  bool is_button_just_pressed(GamepadButtons button) {
...
struct InputBinding {
  ...
  InputBinding& set(BindingType type, Device device, DevicePart device_part, u16 button,
                    u8 is_composite, u8 is_part_of_composite, u8 repeat);
...
struct InputService : public hydra::Service {
  HYDRA_DECLARE_SERVICE(InputService);
  ...
  void add_axis_1d(InputHandle action, DevicePart device, uint16_t axis, float min_deadzone,
                   float max_deadzone);
  void add_axis_2d(InputHandle action, DevicePart device, uint16_t x_axis, uint16_t y_axis,
                   float min_deadzone, float max_deadzone);
  void add_vector_1d(InputHandle action, DevicePart device_pos, uint16_t button_pos,
                     DevicePart device_neg, uint16_t button_neg, bool repeat = true);
  void add_vector_2d(InputHandle action, DevicePart device_up, uint16_t button_up,
                     DevicePart device_down, uint16_t button_down, DevicePart device_left,
                     uint16_t button_left, DevicePart device_right, uint16_t button_right,
                     bool repeat = true);
  ...
  static constexpr cstring k_name = "hydra_input_service";
