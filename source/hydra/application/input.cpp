...
void InputBackend::on_event(void* event_, u8* keys, u32 num_keys, Gamepad* gamepads,
                            u32 num_gamepads, bool& has_focus) {
  SDL_Event& event = *(SDL_Event*)event_;
  switch (event.type) {
    case SDL_KEYDOWN:
    case SDL_KEYUP: {
      i32 key = event.key.keysym.scancode;
      if (key >= 0 && key < (i32)num_keys) keys[key] = (event.type == SDL_KEYDOWN);
...
cstring* gamepad_axis_names() {
  static cstring names[] = {"left_x",       "left_y",        "right_x",           "right_y",
...
bool InputService::is_key_down(Keys key) { return keys[key] && has_focus; }
...
bool InputService::is_mouse_down(MouseButtons button) { return mouse_button[button]; }

bool InputService::is_mouse_clicked(MouseButtons button) {
  return mouse_button[button] && !previous_mouse_button[button];
}

bool InputService::is_mouse_released(MouseButtons button) { return !mouse_button[button]; }

bool InputService::is_mouse_dragging(MouseButtons button) {
  if (!mouse_button[button]) return false;
...
void InputService::add_button(InputHandle action, DevicePart device_part, uint16_t button,
                              bool repeat) {
  const InputAction& binding_action = actions[action];

  InputBinding binding;
  binding.set(BINDING_TYPE_BUTTON, device_from_part(device_part), device_part, button, 0, 0, repeat)
      .set_handles(binding_action.action_map, action);

  bindings.push(binding);
}

void InputService::add_axis_1d(InputHandle action, DevicePart device_part, uint16_t axis,
                               float min_deadzone, float max_deadzone) {
  const InputAction& binding_action = actions[action];

  InputBinding binding;
  binding.set(BINDING_TYPE_AXIS_1D, device_from_part(device_part), device_part, axis, 0, 0, 0)
      .set_deadzones(min_deadzone, max_deadzone)
      .set_handles(binding_action.action_map, action);

  bindings.push(binding);
}

void InputService::add_axis_2d(InputHandle action, DevicePart device_part, uint16_t x_axis,
                               uint16_t y_axis, float min_deadzone, float max_deadzone) {
  const InputAction& binding_action = actions[action];

  InputBinding binding, binding_x, binding_y;
  binding.set(BINDING_TYPE_AXIS_2D, device_from_part(device_part), device_part, u16_max, 1, 0, 0)
      .set_deadzones(min_deadzone, max_deadzone)
      .set_handles(binding_action.action_map, action);
  binding_x.set(BINDING_TYPE_AXIS_2D, device_from_part(device_part), device_part, x_axis, 0, 1, 0)
      .set_deadzones(min_deadzone, max_deadzone)
      .set_handles(binding_action.action_map, action);
  binding_y.set(BINDING_TYPE_AXIS_2D, device_from_part(device_part), device_part, y_axis, 0, 1, 0)
      .set_deadzones(min_deadzone, max_deadzone)
      .set_handles(binding_action.action_map, action);
...
void InputService::add_vector_1d(InputHandle action, DevicePart device_part_pos,
                                 uint16_t button_pos, DevicePart device_part_neg,
                                 uint16_t button_neg, bool repeat) {
  const InputAction& binding_action = actions[action];

  InputBinding binding, binding_positive, binding_negative;
  binding
      .set(BINDING_TYPE_VECTOR_1D, device_from_part(device_part_pos), device_part_pos, u16_max, 1,
           0, repeat)
      .set_handles(binding_action.action_map, action);
  binding_positive
      .set(BINDING_TYPE_VECTOR_1D, device_from_part(device_part_pos), device_part_pos, button_pos,
           0, 1, repeat)
      .set_handles(binding_action.action_map, action);
  binding_negative
      .set(BINDING_TYPE_VECTOR_1D, device_from_part(device_part_neg), device_part_neg, button_neg,
           0, 1, repeat)
      .set_handles(binding_action.action_map, action);
...
void InputService::add_vector_2d(InputHandle action, DevicePart device_part_up, uint16_t button_up,
                                 DevicePart device_part_down, uint16_t button_down,
                                 DevicePart device_part_left, uint16_t button_left,
                                 DevicePart device_part_right, uint16_t button_right, bool repeat) {
  const InputAction& binding_action = actions[action];

  InputBinding binding, binding_up, binding_down, binding_left, binding_right;

  binding
      .set(BINDING_TYPE_VECTOR_2D, device_from_part(device_part_up), device_part_up, u16_max, 1, 0,
           repeat)
      .set_handles(binding_action.action_map, action);
  binding_up
      .set(BINDING_TYPE_VECTOR_2D, device_from_part(device_part_up), device_part_up, button_up, 0,
           1, repeat)
      .set_handles(binding_action.action_map, action);
  binding_down
      .set(BINDING_TYPE_VECTOR_2D, device_from_part(device_part_down), device_part_down,
           button_down, 0, 1, repeat)
      .set_handles(binding_action.action_map, action);
  binding_left
      .set(BINDING_TYPE_VECTOR_2D, device_from_part(device_part_left), device_part_left,
           button_left, 0, 1, repeat)
      .set_handles(binding_action.action_map, action);
  binding_right
      .set(BINDING_TYPE_VECTOR_2D, device_from_part(device_part_right), device_part_right,
           button_right, 0, 1, repeat)
      .set_handles(binding_action.action_map, action);
...
void InputService::update(f32 delta) {
  ...
  for (u32 k = 0; k < bindings.size; k++) {
    ...
    if (input_binding.is_composite) continue;

    input_binding.value = false;

    switch (input_binding.device) {
      case DEVICE_KEYBOARD: {
        bool key_value = input_binding.repeat
                             ? is_key_down((Keys)input_binding.button)
                             : is_key_just_pressed((Keys)input_binding.button, false);
      ...
      case DEVICE_GAMEPAD: {
        ...
        switch (input_binding.device_part) {
          case DEVICE_PART_GAMEPAD_AXIS: {
            ...
            input_binding.value =
                fabs(input_binding.value) < min_deadzone ? 0.0f : input_binding.value;
            input_binding.value = fabs(input_binding.value) > max_deadzone
                                      ? (input_binding.value < 0 ? -1.0f : 1.0f)
                                      : input_binding.value;
          ...
          case DEVICE_PART_GAMEPAD_BUTTONS: {
            ...
            input_binding.value =
                input_binding.repeat
                    ? gamepad.is_button_down((GamepadButtons)input_binding.button)
                    : gamepad.is_button_just_pressed((GamepadButtons)input_binding.button);
            break;
          }
            /*case InputBinding::GAMEPAD_HAT:
            {
  ...
  for (u32 k = 0; k < bindings.size; k++) {
    ...
    if (input_binding.is_part_of_composite) continue;
    ...
    switch (input_binding.type) {
      ...
      case BINDING_TYPE_AXIS_1D: {
        input_action.value.x =
            input_binding.value != 0.f ? input_binding.value : input_action.value.x;
        break;
      }
      ...
      case BINDING_TYPE_AXIS_2D: {
        ...
        input_action.value.x =
            input_binding_x.value != 0.0f ? input_binding_x.value : input_action.value.x;
        input_action.value.y =
            input_binding_y.value != 0.0f ? input_binding_y.value : input_action.value.y;
      ...
      case BINDING_TYPE_VECTOR_1D: {
        ...
        input_action.value.x = input_binding_pos.value   ? input_binding_pos.value
                               : input_binding_neg.value ? -input_binding_neg.value
                                                         : input_action.value.x;
        break;
      }
      ...
      case BINDING_TYPE_VECTOR_2D: {
        ...
        input_action.value.x = input_binding_right.value  ? 1.0f
                               : input_binding_left.value ? -1.0f
                                                          : input_action.value.x;
        input_action.value.y = input_binding_up.value     ? 1.0f
                               : input_binding_down.value ? -1.0f
                                                          : input_action.value.y;
        break;
      }
    }
      ...
      for ( u32 j = 0; j < action_map.num_actions; j++ ) {
          ...
          for ( u32 k = 0; k < input_action.bindings.size; k++ ) {
              ...
              switch ( input_binding.device ) {
                  case DEVICE_KEYBOARD:
                  {
                      bool key_value = input_binding.repeat ? is_key_down(
  (Keys)input_binding.button ) : is_key_just_pressed( (Keys)input_binding.button, false );
                      input_binding.value = key_value ? 1.0f : 0.0f;
                      break;
                  }
                  ...
                  case DEVICE_GAMEPAD:
                  {
                      ...
                      switch ( input_binding.device_part ) {
                          ...
                          {
                              input_binding.value = gamepad.axis[ input_binding.button ];
                              input_binding.value = fabs( input_binding.value ) < min_deadzone ?
  0.0f : input_binding.value; input_binding.value = fabs( input_binding.value ) > max_deadzone ? (
  input_binding.value < 0 ? -1.0f : 1.0f ) : input_binding.value;
          ...
          for ( u32 k = 0; k < input_action.bindings.size; k++ ) {
              ...
              switch ( input_binding.type ) {
                  case BUTTON:
                  {
                      input_action.value.x = fmax( input_action.value.x, input_binding.value ? 1.0f
  : 0.0f ); break;
                  }

                  case AXIS_1D:
                  {
                      input_action.value.x = input_binding.value != 0.f ? input_binding.value :
  input_action.value.x; break;
                  }
                  ...
                  case AXIS_2D:
                  {
                      ...
                      input_action.value.x = input_binding_x.value != 0.0f ? input_binding_x.value :
  input_action.value.x; input_action.value.y = input_binding_y.value != 0.0f ? input_binding_y.value
  : input_action.value.y;
                  ...
                  case VECTOR_1D:
                  {
                      ...
                      input_action.value.x = input_binding_pos.value ? input_binding_pos.value :
  input_binding_neg.value ? -input_binding_neg.value : input_action.value.x; break;
                  }
                  ...
                  case VECTOR_2D:
                  {
                      ...
                      input_action.value.x = input_binding_right.value ? 1.0f :
  input_binding_left.value ? -1.0f : input_action.value.x; input_action.value.y =
  input_binding_up.value ? 1.0f : input_binding_down.value ? -1.0f : input_action.value.y; break;
                  }
              }
          }
      }
  ...
  if (ImGui::Begin("Input")) {
    ...
    if (ImGui::TreeNode("Devices")) {
      ...
      if (ImGui::TreeNode("Mouse")) {
        ...
        for (u32 i = 0; i < MOUSE_BUTTONS_COUNT; i++) {
          ...
          ImGui::SameLine();
          ImGui::Text("Clicked Position     %4.1f,%4.1f", mouse_clicked_position[i].x,
                      mouse_clicked_position[i].y);
          ImGui::SameLine();
    ...
    if (ImGui::TreeNode("Actions")) {
      for (u32 j = 0; j < actions.size; j++) {
        ...
        ImGui::Text("Action %s, x %2.3f y %2.3f", input_action.name, input_action.value.x,
                    input_action.value.y);
      }
    ...
    if (ImGui::TreeNode("Bindings")) {
      for (u32 k = 0; k < bindings.size; k++) {
        ...
        switch (binding.type) {
          case BINDING_TYPE_VECTOR_1D: {
            ImGui::Text(
                "Binding action %s, type %s, value %f, composite %u, part of composite %u, button "
                "%s",
                parent_action.name, "vector 1d", binding.value, binding.is_composite,
                binding.is_part_of_composite, button_name);
            break;
          }
          case BINDING_TYPE_VECTOR_2D: {
            ImGui::Text("Binding action %s, type %s, value %f, composite %u, part of composite %u",
                        parent_action.name, "vector 2d", binding.value, binding.is_composite,
                        binding.is_part_of_composite);
            break;
          }
          case BINDING_TYPE_AXIS_1D: {
            ImGui::Text("Binding action %s, type %s, value %f, composite %u, part of composite %u",
                        parent_action.name, "axis 1d", binding.value, binding.is_composite,
                        binding.is_part_of_composite);
            break;
          }
          case BINDING_TYPE_AXIS_2D: {
            ImGui::Text("Binding action %s, type %s, value %f, composite %u, part of composite %u",
                        parent_action.name, "axis 2d", binding.value, binding.is_composite,
                        binding.is_part_of_composite);
            break;
          }
          case BINDING_TYPE_BUTTON: {
            ImGui::Text(
                "Binding action %s, type %s, value %f, composite %u, part of composite %u, button "
                "%s",
                parent_action.name, "button", binding.value, binding.is_composite,
                binding.is_part_of_composite, button_name);
            break;
          }
        }
...
float InputAction::read_value_1d() const { return value.x; }

InputVector2 InputAction::read_value_2d() const { return value; }

InputBinding& InputBinding::set(BindingType type_, Device device_, DevicePart device_part_,
                                u16 button_, u8 is_composite_, u8 is_part_of_composite_,
                                u8 repeat_) {
  type = type_;
  device = device_;
