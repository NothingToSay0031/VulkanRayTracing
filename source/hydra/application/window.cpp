...
void Window::init(void* configuration_) {
  ...
  window = SDL_CreateWindow(configuration.name, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                            configuration.width, configuration.height, window_flags);
      ...
      case SDL_WINDOWEVENT: {
        switch (event.window.event) {
          ...
          case SDL_WINDOWEVENT_RESIZED: {
            ...
            // NOTE: This goes in an infinite loop when maximising a window that has an odd
            // width/height.
            /*if ( ( event.window.data1 % 2 == 1 ) || ( event.window.data2 % 2 == 1 ) ) {
                u32 new_width = ( u32 )( event.window.data1 % 2 == 0 ? event.window.data1 :
            event.window.data1 - 1 ); u32 new_height = ( u32 )( event.window.data2 % 2 == 0 ?
            event.window.data2 : event.window.data2 - 1 );

                if ( new_width != width || new_height != height ) {
                    SDL_SetWindowSize( window, new_width, new_height );

                    rprint( "Forcing resize to a multiple of 2, %ux%u from %ux%u\n", new_width,
            new_height, event.window.data1, event.window.data2 );
                }
            }*/
            // else
            {
...
void Window::unregister_os_messages_callback(OsMessagesCallback callback) {
  RASSERTM(os_messages_callbacks.size < 8,
