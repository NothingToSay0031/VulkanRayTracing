...
// #define HYDRA_MEMORY_STACK

//
#define HEAP_ALLOCATOR_STATS

#if defined(HYDRA_MEMORY_STACK)
#include "external/StackWalker.h"
#endif  // HYDRA_MEMORY_STACK
...
void MemoryService::init(void* configuration) {
  ...
  MemoryServiceConfiguration* memory_configuration =
      static_cast<MemoryServiceConfiguration*>(configuration);
...
void exit_walker(void* ptr, size_t size, int used, void* user) {
  ...
  if (used) rprint("Found active allocation %p, %llu\n", ptr, size);
}
...
void HeapAllocator::debug_ui() {
  ...
  ImGui::Text("\tAllocated %llu K, free %llu Mb, total %llu Mb",
              ...
              (max_size - stats.allocated_bytes) / (1024 * 1024), max_size / (1024 * 1024));
}
#endif  // HYDRA_IMGUI
...
void* HeapAllocator::allocate(sizet size, sizet alignment) {
  ...
  void* allocated_memory =
      alignment == 1 ? tlsf_malloc(tlsf_handle, size) : tlsf_memalign(tlsf_handle, alignment, size);
...
void LinearAllocator::clear() { allocated_size = 0; }

// Memory Methods /////////////////////////////////////////////////////////
void memory_copy(void* destination, void* source, sizet size) { memcpy(destination, source, size); }
...
// MallocAllocator ///////////////////////////////////////////////////////
void* MallocAllocator::allocate(sizet size, sizet alignment) { return malloc(size); }

void* MallocAllocator::allocate(sizet size, sizet alignment, cstring file, i32 line) {
  return malloc(size);
}

void MallocAllocator::deallocate(void* pointer) { free(pointer); }
...
void StackAllocator::deallocate(void* pointer) {
  ...
  RASSERTM(pointer < memory + total_size,
           "Out of bound free on linear allocator (outside bounds). Tempting to free %p, %llu "
           "after beginning of buffer (memory %p size %llu, allocated %llu)",
           (u8*)pointer, (u8*)pointer - memory, memory, total_size, allocated_size);
  RASSERTM(pointer < memory + allocated_size,
           "Out of bound free on linear allocator (inside bounds, after allocated). Tempting to "
           "free %p, %llu after beginning of buffer (memory %p size %llu, allocated %llu)",
           (u8*)pointer, (u8*)pointer - memory, memory, total_size, allocated_size);
...
sizet StackAllocator::get_marker() { return allocated_size; }
...
void DoubleStackAllocator::deallocate(void* pointer) { RASSERT(false); }
...
sizet DoubleStackAllocator::get_top_marker() { return top; }

sizet DoubleStackAllocator::get_bottom_marker() { return bottom; }
...
void DoubleStackAllocator::clear_top() { top = total_size; }

void DoubleStackAllocator::clear_bottom() { bottom = 0; }
