...
static const char* kDefault3DModel =
    "../../deps/src/glTF-Sample-Models/2.0/Sponza/glTF/Sponza.gltf";

#define InjectDefault3DModel()                                                                    \
  if (hydra::file_exists(kDefault3DModel)) {                                                      \
    argc = 2;                                                                                     \
    argv[1] = const_cast<char*>(kDefault3DModel);                                                 \
  } else {                                                                                        \
    printf(                                                                                       \
        "Unable to find default model. Please check the README in the root folder and make sure " \
        "you've run `python ./bootstrap.py` to download all the additional assets for this "      \
        "project.\n");                                                                            \
    exit(-1);                                                                                     \
  }
...
struct AnimationSampler {
  ...
  i32 output_keyframe_buffer_index;  // "The index of an accessor, containing keyframe output
                                     // values."
