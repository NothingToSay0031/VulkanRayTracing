...
FlatHashMapIterator FlatHashMap<K, V>::find(const K& key) {
  ...
  while (true) {
    ...
    for (int i : group.Match(hash2)) {
      ...
      if (key_value.key == key) return {sequence.get_offset(i)};
    }
...
u32 FlatHashMap<K, V>::remove(const K& key) {
  ...
  if (iterator.index == k_iterator_end) return 0;
...
FindResult FlatHashMap<K, V>::find_or_prepare_insert(const K& key) {
  ...
  while (true) {
    ...
    for (int i : group.Match(hash_2(hash))) {
      const KeyValue& key_value = *(slots_ + sequence.get_offset(i));
      if (key_value.key == key) return {sequence.get_offset(i), false};
    }
...
V& FlatHashMap<K, V>::get(const K& key) {
  FlatHashMapIterator iterator = find(key);
  if (iterator.index != k_iterator_end) return slots_[iterator.index].value;
...
typename FlatHashMap<K, V>::KeyValue& FlatHashMap<K, V>::get_structure(const K& key) {
  FlatHashMapIterator iterator = find(key);
  if (iterator.index != k_iterator_end) return slots_[iterator.index];
  return default_key_value;
}

template <typename K, typename V>
typename FlatHashMap<K, V>::KeyValue& FlatHashMap<K, V>::get_structure(
    const FlatHashMapIterator& iterator) {
...
inline void FlatHashMap<K, V>::iterator_skip_empty_or_deleted(FlatHashMapIterator& it) {
  ...
  if (*ctrl == k_control_bitmask_sentinel) it.index = k_iterator_end;
}
...
inline i8* group_init_empty() {
  alignas(16) static constexpr i8 empty_group[] = {
      k_control_bitmask_sentinel, k_control_bitmask_empty, k_control_bitmask_empty,
      k_control_bitmask_empty,    k_control_bitmask_empty, k_control_bitmask_empty,
      k_control_bitmask_empty,    k_control_bitmask_empty, k_control_bitmask_empty,
      k_control_bitmask_empty,    k_control_bitmask_empty, k_control_bitmask_empty,
      k_control_bitmask_empty,    k_control_bitmask_empty, k_control_bitmask_empty,
      k_control_bitmask_empty};
...
inline u64 ProbeSequence::get_offset() const { return offset; }

inline u64 ProbeSequence::get_offset(u64 i) const { return (offset + i) & mask; }

inline u64 ProbeSequence::get_index() const { return index; }
