...
void StringBuffer::append(const char* string) { append_f("%s", string); }
...
#if defined(_MSC_VER)
  int written_chars =
      vsnprintf_s(&data[current_size], buffer_size - current_size, _TRUNCATE, format, args);
#else
...
void StringBuffer::append(const StringView& text) {
  const sizet max_length =
      current_size + text.length < buffer_size ? text.length : buffer_size - current_size;
...
char* StringBuffer::append_use(const char* string) { return append_use_f("%s", string); }
...
char* StringBuffer::reserve(sizet size) {
  if (current_size + size >= buffer_size) return nullptr;
...
void StringArray::init(u32 size, Allocator* allocator_) {
  ...
  char* allocated_memory = (char*)allocator_->allocate(
      size + sizeof(FlatHashMap<u64, u32>) + sizeof(FlatHashMapIterator), 1);
...
sizet StringArray::get_string_count() const { return string_to_index->size; }
...
bool StringArray::has_next_string(FlatHashMapIterator* it) const { return it->is_valid(); }
...
cstring StringArray::intern(cstring string) {
  ...
  const sizet hashed_string = hydra::hash_bytes((void*)string, length, seed);
