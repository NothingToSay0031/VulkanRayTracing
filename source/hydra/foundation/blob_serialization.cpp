...
void BlobSerializer::shutdown() {
  if (is_reading) {
    ...
    if (blob_memory && has_allocated_memory) rfree(blob_memory, allocator);
  } else {
    if (blob_memory) rfree(blob_memory, allocator);
  }
...
#if (_MSC_VER)
  int written_chars = vsnprintf_s(&destination_memory[allocated_offset],
                                  total_size - allocated_offset, _TRUNCATE, format, args);
#else
  int written_chars =
      vsnprintf(&destination_memory[allocated_offset], total_size - allocated_offset, format, args);
#endif
...
i32 BlobSerializer::get_relative_data_offset(void* data) {
  // data_memory points to the newly allocated data structure to be used at
  // runtime.
