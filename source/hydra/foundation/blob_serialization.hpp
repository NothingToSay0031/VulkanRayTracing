...
struct BlobSerializer {
  ...
  template <typename T>
  T* read(Allocator* allocator, u32 serializer_version, sizet size, char* blob_memory,
  ...
  char* allocate_static(
      sizet size);  // Just allocate size bytes and return. Used to fill in structures.
  ...
  template <typename T>
  void allocate_and_set(
      RelativeArray<T>& data, u32 num_elements,
      void* source_data = nullptr);  // Allocate an array and set it so it can be accessed.

  void allocate_and_set(RelativeString& string, cstring format,
                        ...);  // Allocate and set a static string.
  void allocate_and_set(RelativeString& string, char* text,
                        u32 length);  // Allocate and set a static string.
...
template <typename T>
void BlobSerializer::write_and_serialize(Allocator* allocator_, u32 serializer_version_, sizet size,
                                         T* data) {
...
template <typename T>
T* BlobSerializer::read(Allocator* allocator_, u32 serializer_version_, sizet size,
                        char* blob_memory_, bool force_serialization) {
...
inline void BlobSerializer::allocate_and_set(RelativePointer<T>& data, void* source_data) {
  ...
  if (source_data) {
    hydra::memory_copy(destination_memory, source_data, sizeof(T));
  }
}

template <typename T>
inline void BlobSerializer::allocate_and_set(RelativeArray<T>& data, u32 num_elements,
                                             void* source_data) {
  char* destination_memory = allocate_static(sizeof(T) * num_elements);
  data.set(destination_memory, num_elements);

  if (source_data) {
    hydra::memory_copy(destination_memory, source_data, sizeof(T) * num_elements);
  }
}
...
inline void BlobSerializer::serialize(Array<T>* data) {
  if (is_reading) {
    ...
    //
    serialized_offset =
        cached_serialized + source_data_offset - sizeof(u32);  // -sizeof( u64 ) * 2;
