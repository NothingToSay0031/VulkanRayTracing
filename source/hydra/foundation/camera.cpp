...
void Camera::init_orthographic(f32 near_plane_, f32 far_plane_, f32 viewport_width_,
                               f32 viewport_height_, f32 zoom_) {
  perspective = false;
...
void Camera::calculate_projection_matrix() {
  if (perspective) {
    ...
    projection = glms_ortho(zoom * -viewport_width / 2.f, zoom * viewport_width / 2.f,
                            zoom * -viewport_height / 2.f, zoom * viewport_height / 2.f, near_plane,
                            far_plane);
  }
}

void Camera::calculate_view_projection() { view_projection = glms_mat4_mul(projection, view); }

vec3s Camera::unproject(const vec3s& screen_coordinates) {
  return glms_unproject(screen_coordinates, view_projection,
                        {0, 0, viewport_width, viewport_height});
}

vec3s Camera::unproject_inverted_y(const vec3s& screen_coordinates) {
  const vec3s screen_coordinates_y_inv{screen_coordinates.x, viewport_height - screen_coordinates.y,
