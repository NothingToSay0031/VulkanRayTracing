...
static void try_load_string(json& json_data, cstring key, StringBuffer& string_buffer,
                            Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) return;
...
static void try_load_int_array(json& json_data, cstring key, u32& count, i32** array,
                               Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) {
    count = 0;
...
static void try_load_float_array(json& json_data, cstring key, u32& count, float** array,
                                 Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) {
    count = 0;
...
static void load_buffers(json& json_data, glTF::glTF& gltf_data, Allocator* allocator) {
  ...
  gltf_data.buffers =
      (glTF::Buffer*)allocate_and_zero(allocator, sizeof(glTF::Buffer) * buffer_count);
...
static void load_buffer_views(json& json_data, glTF::glTF& gltf_data, Allocator* allocator) {
  ...
  sizet buffer_count = buffers.size();
  gltf_data.buffer_views =
      (glTF::BufferView*)allocate_and_zero(allocator, sizeof(glTF::BufferView) * buffer_count);
...
static void load_node(json& json_data, glTF::Node& node, Allocator* allocator) {
  ...
  try_load_float_array(json_data, "translation", node.translation_count, &node.translation,
                       allocator);
...
static void load_mesh_primitive(json& json_data, glTF::MeshPrimitive& mesh_primitive,
                                Allocator* allocator) {
  ...
  mesh_primitive.attributes = (glTF::MeshPrimitive::Attribute*)allocate_and_zero(
      allocator, sizeof(glTF::MeshPrimitive::Attribute) * attributes.size());
...
static void load_mesh_primitives(json& json_data, glTF::Mesh& mesh, Allocator* allocator) {
  ...
  sizet array_count = array.size();
  mesh.primitives =
      (glTF::MeshPrimitive*)allocate_and_zero(allocator, sizeof(glTF::MeshPrimitive) * array_count);
...
static void load_accessors(json& json_data, glTF::glTF& gltf_data, Allocator* allocator) {
  ...
  sizet array_count = array.size();
  gltf_data.accessors =
      (glTF::Accessor*)allocate_and_zero(allocator, sizeof(glTF::Accessor) * array_count);
...
static void try_load_TextureInfo(json& json_data, cstring key, glTF::TextureInfo** texture_info,
                                 Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) {
...
static void try_load_MaterialNormalTextureInfo(json& json_data, cstring key,
                                               glTF::MaterialNormalTextureInfo** texture_info,
                                               Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) {
    *texture_info = nullptr;
  ...
  glTF::MaterialNormalTextureInfo* ti = (glTF::MaterialNormalTextureInfo*)allocator->allocate(
...
static void try_load_MaterialOcclusionTextureInfo(json& json_data, cstring key,
                                                  glTF::MaterialOcclusionTextureInfo** texture_info,
                                                  Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) {
    *texture_info = nullptr;
  ...
  glTF::MaterialOcclusionTextureInfo* ti = (glTF::MaterialOcclusionTextureInfo*)allocator->allocate(
...
static void try_load_MaterialPBRMetallicRoughness(json& json_data, cstring key,
                                                  glTF::MaterialPBRMetallicRoughness** texture_info,
                                                  Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) {
    *texture_info = nullptr;
  ...
  glTF::MaterialPBRMetallicRoughness* ti = (glTF::MaterialPBRMetallicRoughness*)allocator->allocate(
      sizeof(glTF::MaterialPBRMetallicRoughness), 64);

  try_load_float_array(*it, "baseColorFactor", ti->base_color_factor_count, &ti->base_color_factor,
                       allocator);
...
static void load_material(json& json_data, glTF::Material& material, Allocator* allocator) {
  try_load_float_array(json_data, "emissiveFactor", material.emissive_factor_count,
                       &material.emissive_factor, allocator);
  ...
  try_load_MaterialNormalTextureInfo(json_data, "normalTexture", &material.normal_texture,
                                     allocator);
  try_load_MaterialOcclusionTextureInfo(json_data, "occlusionTexture", &material.occlusion_texture,
                                        allocator);
  try_load_MaterialPBRMetallicRoughness(json_data, "pbrMetallicRoughness",
                                        &material.pbr_metallic_roughness, allocator);
...
static void load_materials(json& json_data, glTF::glTF& gltf_data, Allocator* allocator) {
  ...
  sizet array_count = array.size();
  gltf_data.materials =
      (glTF::Material*)allocate_and_zero(allocator, sizeof(glTF::Material) * array_count);
...
static void load_textures(json& json_data, glTF::glTF& gltf_data, Allocator* allocator) {
  ...
  sizet array_count = array.size();
  gltf_data.textures =
      (glTF::Texture*)allocate_and_zero(allocator, sizeof(glTF::Texture) * array_count);
...
static void load_samplers(json& json_data, glTF::glTF& gltf_data, Allocator* allocator) {
  ...
  sizet array_count = array.size();
  gltf_data.samplers =
      (glTF::Sampler*)allocate_and_zero(allocator, sizeof(glTF::Sampler) * array_count);
...
static void load_animation(json& json_data, glTF::Animation& animation, Allocator* allocator) {
  ...
  if (json_array.is_array()) {
    ...
    glTF::AnimationSampler* values = (glTF::AnimationSampler*)allocate_and_zero(
        allocator, sizeof(glTF::AnimationSampler) * count);
  ...
  if (json_array.is_array()) {
    ...
    glTF::AnimationChannel* values = (glTF::AnimationChannel*)allocate_and_zero(
        allocator, sizeof(glTF::AnimationChannel) * count);
...
static void load_animations(json& json_data, glTF::glTF& gltf_data, Allocator* allocator) {
  ...
  gltf_data.animations =
      (glTF::Animation*)allocate_and_zero(allocator, sizeof(glTF::Animation) * array_count);
...
void gltf_free(glTF::glTF& scene) { scene.allocator.shutdown(); }

i32 gltf_get_attribute_accessor_index(glTF::MeshPrimitive::Attribute* attributes,
                                      u32 attribute_count, cstring attribute_name) {
...
i32 hydra::glTF::get_data_offset(i32 accessor_offset, i32 buffer_view_offset) {
