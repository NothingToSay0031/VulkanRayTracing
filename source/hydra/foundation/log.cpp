...
static void output_console(char* log_buffer_) { printf("%s", log_buffer_); }

#if defined(_MSC_VER)
static void output_visual_studio(char* log_buffer_) { OutputDebugStringA(log_buffer_); }
#endif
...
void LogService::print_format(cstring format, ...) {
  ...
  if (print_callback) print_callback(log_buffer);
}

void LogService::set_callback(PrintCallback callback) { print_callback = callback; }
