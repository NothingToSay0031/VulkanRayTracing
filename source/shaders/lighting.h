...
#ifndef HYDRA_GLSL_LIGHTING_H
#define HYDRA_GLSL_LIGHTING_H
...
layout(set = MATERIAL_SET, binding = 20) readonly buffer ZBins { uint bins[]; };

layout(set = MATERIAL_SET, binding = 21) readonly buffer Lights { Light lights[]; };

layout(set = MATERIAL_SET, binding = 22) readonly buffer Tiles { uint tiles[]; };
...
layout(set = MATERIAL_SET, binding = 25) readonly buffer LightIndices { uint light_indices[]; };
...
float f_schlick_f90(float u, float f0, float f90) { return f0 + (f90 - f0) * pow(1.0 - u, 5.0); }
...
float vector_to_depth_value(vec3 direction, float radius, float rcp_n_minus_f) {
  ...
  const float normalized_z_component =
      (n * f * rcp_n_minus_f) / local_z_component - f * rcp_n_minus_f;
...
float get_directional_light_visibility(vec3 light_position, uint sample_count, vec3 world_position,
                                       vec3 normal, uint frame_index) {
  ...
  mat3 to_local_coord = mat3(x_axis, l, z_axis);
#endif

  vec3 x_axis = l.y == 1.0f ? normalize(cross(l, vec3(0.0f, 0.0f, 1.0f)))
                            : normalize(cross(l, vec3(0.0f, 1.0f, 0.0f)));
  ...
  if (NoL > 0.001f) {
    for (uint s = 0; s < sample_count; ++s) {
      ...
      rayQueryInitializeEXT(rayQuery, as, gl_RayFlagsOpaqueEXT | gl_RayFlagsTerminateOnFirstHitEXT,
                            0xff, world_position, 0.05, random_dir, 100.0f);
...
float get_point_light_visibility(uint light_index, uint sample_count, vec3 world_position,
                                 vec3 normal, uint frame_index) {
  ...
  if ((NoL > 0.001f) && (d <= r) && (attenuation > 0.001f)) {
    for (uint s = 0; s < sample_count; ++s) {
      ...
      rayQueryEXT rayQuery;
      rayQueryInitializeEXT(rayQuery, as, gl_RayFlagsOpaqueEXT | gl_RayFlagsTerminateOnFirstHitEXT,
                            0xff, world_position, 0.05, random_dir, d);
      rayQueryProceedEXT(rayQuery);
...
vec3 calculate_directional_light_contribution(vec4 albedo, float roughness, vec3 normal,
                                              vec3 emissive, vec3 world_position, vec3 v, vec3 F0,
                                              float NoV, uvec2 screen_uv, vec3 light_direction) {
  ...
  float shadow = texture(global_textures_3d[shadow_visibility_texture_index],
                         vec3(screen_uv * (1 / resolution), 0))
                     .r;
  ...
  if (shadow >= 0.0f && NoL > 0.0001f) {
    ...
    pixel_luminance = (diffuse + specular) * light_intensity *
                      unpack_color_rgba(raytraced_shadow_light_color_type).rgb;
  }

  return pixel_luminance;
}

vec3 calculate_raytraced_point_light_contribution(vec4 albedo, float roughness, vec3 normal,
                                                  vec3 emissive, vec3 world_position, vec3 v,
                                                  vec3 F0, float NoV, uvec2 screen_pixels) {
  const vec3 position_to_light = raytraced_shadow_light_position - world_position;
  const vec3 l = normalize(position_to_light);
  const float NoL = clamp(dot(normal, l), 0.0, 1.0);
  ...
  float shadow = texelFetch(global_textures_3d[shadow_visibility_texture_index],
                            ivec3(screen_pixels * 0.5f, 0), 0)
                     .r;
...
vec3 calculate_point_light_contribution(vec4 albedo, float roughness, vec3 normal, vec3 emissive,
                                        ...
                                        uvec2 screen_uv, uint shadow_light_index) {
  ...
  const float current_depth =
      vector_to_depth_value(shadow_position_to_light, light.radius, light.rcp_n_minus_f);
  ...
  for (uint i = 0; i < samples; ++i) {
    ...
    const float closest_depth =
        texture(global_textures_cubemaps_array[nonuniformEXT(cubemap_shadows_index)],
                vec4(sampling_position, shadow_light_index))
            .r;
  ...
  const float closest_depth =
      texture(global_textures_cubemaps_array[nonuniformEXT(cubemap_shadows_index)],
              vec4(shadow_position_to_light, shadow_light_index))
          .r;
...
uint get_lowest_bit(uint v) {
  uint bit_position_lookup[32] = {0,  1,  28, 2,  29, 14, 24, 3, 30, 22, 20, 15, 25, 17, 4,  8,
...
vec4 calculate_lighting(vec4 base_colour, vec3 orm, vec3 normal, vec3 emissive, vec3 world_position,
                        uvec2 position, vec2 screen_uv, bool transparent) {
  ...
  // NOTE(marco): get the minimum and maximum light index across all threads of the wave. From this
  // point, these values are stored in scalar registers and we avoid storing them in vector
  // registers
  ...
  for (uint word_index = word_min; word_index <= word_max; ++word_index) {
    ...
    while (merged_mask != 0) {
      ...
      final_color.rgb +=
          calculate_point_light_contribution(albedo, roughness, normal, emissive, world_position, V,
                                             F0, NoV, position, global_light_index);
    }
  }
#else
  if (min_light_id != NUM_LIGHTS + 1) {
    for (uint light_id = min_light_id; light_id <= max_light_id; ++light_id) {
      ...
      if ((tiles[address + word_id] & (1 << bit_id)) != 0) {
        ...
        final_color.rgb +=
            calculate_point_light_contribution(albedo, roughness, normal, emissive, world_position,
                                               V, F0, NoV, position, global_light_index);
      }
    }
  }
  ...
  if (!transparent) {
    ...
    if (is_raytrace_shadow_point_light()) {
      final_color.rgb += calculate_raytraced_point_light_contribution(
          albedo, roughness, normal, emissive, world_position, V, F0, NoV, position);
    } else {
      // Directional light
  ...
  vec2 upsampling_weights =
      textureLod(global_textures[nonuniformEXT(bilateral_weights_texture_index)], screen_uv, 0).rg;
  vec3 indirect_irradiance =
      textureLod(global_textures[nonuniformEXT(indirect_lighting_texture_index)], screen_uv, 0).rgb;
  ...
  vec2 envBRDF =
      textureLod(global_textures[nonuniformEXT(brdf_lut_texture_index)], vec2(NoV, roughness), 0)
          .rg;
  ...
  if (debug_show_tiles > 0) {
    ...
    final_color.rgb *=
        vec3(float(mhash & 255), float((mhash >> 8) & 255), float((mhash >> 16) & 255)) / 255.0;
  }

  if (debug_show_bins > 0) {
    uint bin_hash = hash(bin_index);
    final_color.rgb =
        vec3(float(bin_hash & 255), float((bin_hash >> 8) & 255), float((bin_hash >> 16) & 255)) /
        255.0;
  }
