...
#ifndef HYDRA_GLSL_CULLING_H
#define HYDRA_GLSL_CULLING_H
...
bool occlusion_cull(vec3 view_bounding_center, float radius, float z_near, float projection_00,
                    float projection_11, uint depth_pyramid_texture_index,
                    vec3 world_bounding_center, vec3 camera_world_position,
  ...
  if (project_sphere(view_bounding_center, radius, z_near, projection_00, projection_11, aabb)) {
    ...
    ivec2 depth_pyramid_size =
        textureSize(global_textures[nonuniformEXT(depth_pyramid_texture_index)], 0);
    ...
    float depth =
        textureLod(global_textures[nonuniformEXT(depth_pyramid_texture_index)], uv, level).r;
    // Sample also 4 corners
    depth = max(depth, textureLod(global_textures[nonuniformEXT(depth_pyramid_texture_index)],
                                  vec2(aabb.x, 1.0f - aabb.y), level)
                           .r);
    depth = max(depth, textureLod(global_textures[nonuniformEXT(depth_pyramid_texture_index)],
                                  vec2(aabb.z, 1.0f - aabb.w), level)
                           .r);
    depth = max(depth, textureLod(global_textures[nonuniformEXT(depth_pyramid_texture_index)],
                                  vec2(aabb.x, 1.0f - aabb.w), level)
                           .r);
    depth = max(depth, textureLod(global_textures[nonuniformEXT(depth_pyramid_texture_index)],
                                  vec2(aabb.z, 1.0f - aabb.y), level)
                           .r);

    // vec3 camera_world_position = freeze_occlusion_camera() ? camera_position.xyz :
    // camera_position_debug.xyz;
    vec3 dir = normalize(camera_world_position - world_bounding_center.xyz);
    // vec4 sceen_space_center_last = early_late_flag == 0 ? previous_view_projection *
    // vec4(world_bounding_center + dir * radius, 1.0) : view_projection *
    // vec4(world_bounding_center + dir * radius, 1.0);
    vec4 sceen_space_center_last =
        culling_view_projection * vec4(world_bounding_center + dir * radius, 1.0);
    ...
    // debug_draw_box( world_center.xyz - vec3(radius), world_center.xyz + vec3(radius),
    // vec4(1,1,1,0.5)); debug_draw_box( aabb_world.xyz - vec3(radius * 1.1), aabb_world.xyz +
    // vec3(radius * 1.1), vec4(0,depth_sphere - depth,1,0.5));

    // debug_draw_2d_box(aabb.xy * 2.0 - 1, aabb.zw * 2 - 1, occlusion_visible ? vec4(0,1,0,1) :
    // vec4(1,0,0,1));
  }
...
uint get_cube_face_mask(vec3 cube_map_pos, vec3 aabb_min, vec3 aabb_max) {
  vec3 plane_normals[] = {vec3(-1, 1, 0), vec3(1, 1, 0), vec3(1, 0, 1),
                          vec3(1, 0, -1), vec3(0, 1, 1), vec3(0, -1, 1)};
  vec3 abs_plane_normals[] = {vec3(1, 1, 0), vec3(1, 1, 0), vec3(1, 0, 1),
                              vec3(1, 0, 1), vec3(0, 1, 1), vec3(0, 1, 1)};
