...
struct AsynchronousLoadTask : enki::IPinnedTask {
  ...
  hydra::AsynchronousLoader* async_loader;
  enki::TaskScheduler* task_scheduler;
  bool execute = true;
...
static void test_sphere_aabb(hydra::GameCamera& game_camera) {
  ...
  vec4s aabb2{aabb.x * 0.5f + 0.5f, aabb.w * -0.5f + 0.5f, aabb.z * 0.5f + 0.5f,
              aabb.y * -0.5f + 0.5f};

  vec3s left, right, top, bottom;
  hydra::get_bounds_for_axis(vec3s{1, 0, 0}, {view_space_pos.x, view_space_pos.y, view_space_pos.z},
                             radius, game_camera.camera.near_plane, left, right);
  hydra::get_bounds_for_axis(vec3s{0, 1, 0}, {view_space_pos.x, view_space_pos.y, view_space_pos.z},
                             radius, game_camera.camera.near_plane, top, bottom);

  left = hydra::project(game_camera.camera.projection, left);
  right = hydra::project(game_camera.camera.projection, right);
  top = hydra::project(game_camera.camera.projection, top);
  bottom = hydra::project(game_camera.camera.projection, bottom);
  ...
  rprint("Camera visible %u, x %f, %f, widh %f --- %f,%f width %f\n", camera_visible ? 1 : 0,
         aabb2.x, aabb2.z, aabb2.z - aabb2.x, left.x * 0.5 + 0.5, right.x * 0.5 + 0.5,
         (left.x - right.x) * 0.5);
  rprint("y %f, %f, height %f --- %f,%f height %f\n", aabb2.y, aabb2.w, aabb2.w - aabb2.y,
         top.y * 0.5 + 0.5, bottom.y * 0.5 + 0.5, (top.y - bottom.y) * 0.5);
}

// Light placement function ///////////////////////////////////////////////
void place_lights(hydra::Array<hydra::Light>& lights, u32 active_lights, bool grid) {
  using namespace hydra;

  if (grid) {
    const u32 lights_per_side = hydra::ceilu32(sqrtf(active_lights * 1.f));
...
static void perform_geometric_tests(bool enable_aabb_cubemap_test, hydra::RenderScene* scene,
                                    const vec3s& aabb_test_position,
                                    hydra::GpuSceneData& scene_data, bool freeze_occlusion_camera,
                                    hydra::GameCamera& game_camera, bool enable_light_tile_debug,
                                    hydra::Allocator* allocator, bool enable_light_cluster_debug) {
  using namespace hydra;
  ...
  if (enable_aabb_cubemap_test) {
    ...
    scene->debug_renderer.aabb(glms_vec3_sub(cubemap_position, cubemap_half_size),
                               glms_vec3_add(cubemap_position, cubemap_half_size), {Color::blue});

    vec3s aabb[] = {glms_vec3_subs(aabb_test_position, 0.2f),
                    glms_vec3_adds(aabb_test_position, 0.2f)};
    u32 res = get_cube_face_mask(cubemap_position, aabb);
    // Positive X
    if ((res & 1)) {
      scene->debug_renderer.aabb(glms_vec3_add(cubemap_position, {1, 0, 0}),
                                 glms_vec3_add(cubemap_position, {1.2, .2, .2}),
                                 {Color::get_distinct_color(0)});
    }
    // Negative X
    if ((res & 2)) {
      scene->debug_renderer.aabb(glms_vec3_add(cubemap_position, {-1, 0, 0}),
                                 glms_vec3_add(cubemap_position, {-1.2, -.2, -.2}),
                                 {Color::get_distinct_color(1)});
    }
    // Positive Y
    if ((res & 4)) {
      scene->debug_renderer.aabb(glms_vec3_add(cubemap_position, {0, 1, 0}),
                                 glms_vec3_add(cubemap_position, {.2, 1.2, .2}),
                                 {Color::get_distinct_color(2)});
    }
    // Negative Y
    if ((res & 8)) {
      scene->debug_renderer.aabb(glms_vec3_add(cubemap_position, {0, -1, 0}),
                                 glms_vec3_add(cubemap_position, {.2, -1.2, .2}),
                                 {Color::get_distinct_color(3)});
    }
    // Positive Z
    if ((res & 16)) {
      scene->debug_renderer.aabb(glms_vec3_add(cubemap_position, {0, 0, 1}),
                                 glms_vec3_add(cubemap_position, {.2, .2, 1.2}),
                                 {Color::get_distinct_color(4)});
    }
    // Negative Z
    if ((res & 32)) {
      scene->debug_renderer.aabb(glms_vec3_add(cubemap_position, {0, 0, -1}),
                                 glms_vec3_add(cubemap_position, {.2, .2, -1.2}),
                                 {Color::get_distinct_color(5)});
    }
  ...
  if (false) {
    ...
    auto screen_to_view = [&](const vec4s& screen_pos) -> vec3s {
      // Convert to NDC
      vec2s text_coord{screen_pos.x / scene_data.resolution_x,
                       screen_pos.y / scene_data.resolution_y};

      // Convert to clipSpace
      vec4s clip = vec4s{text_coord.x * 2.0f - 1.0f, (1.0f - text_coord.y) * 2.0f - 1.0f,
    ...
    for (u32 z = 0; z < z_count; ++z) {
      for (u32 y = 0; y < tile_y_count; ++y) {
        for (u32 x = 0; x < tile_x_count; ++x) {
          ...
          vec4s max_point_screen =
              vec4s{f32((x + 1) * tile_size), f32((y + 1) * tile_size), 0.0f, 1.0f};  // Top Right

          vec4s min_point_screen =
              vec4s{f32(x * tile_size), f32(y * tile_size), 0.0f, 1.0f};  // Top Right
          ...
          vec3s min_point_aabb_view = glms_vec3_minv(glms_vec3_minv(min_point_near, min_point_far),
                                                     glms_vec3_minv(max_point_near, max_point_far));
          vec3s max_point_aabb_view = glms_vec3_maxv(glms_vec3_maxv(min_point_near, min_point_far),
                                                     glms_vec3_maxv(max_point_near, max_point_far));

          vec4s min_point_aabb_world{min_point_aabb_view.x, min_point_aabb_view.y,
                                     min_point_aabb_view.z, 1.0f};
          vec4s max_point_aabb_world{max_point_aabb_view.x, max_point_aabb_view.y,
                                     max_point_aabb_view.z, 1.0f};
          ...
          for (u32 l = 0; l < scene->active_lights; ++l) {
            ...
            f32 minx = min(min(light_aabb_min.x, light_aabb_max.x),
                           min(min_point_aabb_view.x, max_point_aabb_view.x));
            f32 miny = min(min(light_aabb_min.y, light_aabb_max.y),
                           min(min_point_aabb_view.y, max_point_aabb_view.y));
            f32 minz = min(min(light_aabb_min.z, light_aabb_max.z),
                           min(min_point_aabb_view.z, max_point_aabb_view.z));

            f32 maxx = max(max(light_aabb_min.x, light_aabb_max.x),
                           max(min_point_aabb_view.x, max_point_aabb_view.x));
            f32 maxy = max(max(light_aabb_min.y, light_aabb_max.y),
                           max(min_point_aabb_view.y, max_point_aabb_view.y));
            f32 maxz = max(max(light_aabb_min.z, light_aabb_max.z),
            ...
            if (intersects) {
              ...
              vec4s sphere_world{light.world_position.x, light.world_position.y,
        ...
        if (light.shadow_map_resolution != 0.0f) {
          {
            ...
            scene->debug_renderer.line_2d(top_left, bottom_right,
                                          {Color::get_distinct_color(l + 1)});
            scene->debug_renderer.line_2d(top_right, bottom_left,
                                          {Color::get_distinct_color(l + 1)});
          }

          {
            vec2s screen_scale{1.0f / f32(scene_data.resolution_x),
                               1.0f / (scene_data.resolution_y)};

            vec2s bottom_right{f32((light.tile_x + 1) * tile_size),
                               f32(scene_data.resolution_y - (light.tile_y + 1) * tile_size)};
            bottom_right = glms_vec2_subs(
                glms_vec2_scale(glms_vec2_mul(bottom_right, screen_scale), 2.0f), 1.0f);

            vec2s top_left{f32((light.tile_x) * tile_size),
                           f32(scene_data.resolution_y - (light.tile_y) * tile_size)};
            top_left =
            ...
            scene->debug_renderer.line_2d(top_right, bottom_right,
                                          {Color::get_distinct_color(l + 1)});
            scene->debug_renderer.line_2d(bottom_left, bottom_right,
                                          {Color::get_distinct_color(l + 1)});
            scene->debug_renderer.line_2d(bottom_left, top_left,
                                          {Color::get_distinct_color(l + 1)});
          }
        }
      }
...
enum Enum { Halton = 0, R2, Hammersley, InterleavedGradients };
...
int main(int argc, char** argv) {
  if (argc < 2) {
    printf("Usage: VulkanRayTracing [path to glTF model]\n");
  ...
  // window
  WindowConfiguration wconf{1280, 800, "Vulkan Ray Tracing",
  ...
  dc.set_window(window.width, window.height, window.platform_handle)
      .set_allocator(&MemoryService::instance()->system_allocator)
  ...
  cstring shader_binaries_folder =
      temporary_name_buffer.append_use_f("%s/shaders/", HYDRA_DATA_FOLDER);
  ...
  if (gpu.fragment_shading_rate_present) {
    ...
    u32 adjusted_width = (window.width + gpu.min_fragment_shading_rate_texel_size.width - 1) /
                         gpu.min_fragment_shading_rate_texel_size.width;
    u32 adjusted_height = (window.height + gpu.min_fragment_shading_rate_texel_size.height - 1) /
    ...
    texture_creation.set_size(adjusted_width, adjusted_height, 1)
        ...
        .set_mips(1)
        .set_layers(1)
        .set_flags(TextureFlags::Compute_mask | TextureFlags::ShadingRate_mask)
        .set_name("fragment_shading_rate");

    scene->fragment_shading_rate_image = gpu.create_texture(texture_creation);

    FrameGraphResourceInfo resource_info{};
    resource_info.set_external_texture_2d(adjusted_width, adjusted_height, VK_FORMAT_R8_UINT, 0,
                                          scene->fragment_shading_rate_image);
    frame_graph.add_resource("shading_rate_image", FrameGraphResourceType_ShadingRate,
                             resource_info);
  }

  static bool use_shader_cache = true;
  static cstring techniques[] = {"reflections.json", "ddgi.json",   "meshlet.json",
                                 "fullscreen.json",  "main.json",   "pbr_lighting.json",
                                 "debug.json",       "culling.json"};
  ...
  auto load_technique = [&](cstring technique_name, bool& shader_changed) {
    temporary_name_buffer.clear();
    cstring path = temporary_name_buffer.append_use_f("%s/%s", HYDRA_SHADER_FOLDER, technique_name);
  ...
  {
    cstring frame_graph_path =
        temporary_name_buffer.append_use_f("%s/%s", HYDRA_WORKING_FOLDER, "graph_ray_tracing.json");
    ...
    if (point_shadows_pass_node) {
      ...
      if (render_pass) {
        render_pass->output.reset().depth(VK_FORMAT_D16_UNORM,
    ...
    sampler_creation
        ...
        .set_name("repeat_sampler");
    repeat_sampler = gpu.create_sampler(sampler_creation);

    sampler_creation
        .set_min_mag_mip(VK_FILTER_NEAREST, VK_FILTER_NEAREST, VK_SAMPLER_MIPMAP_MODE_NEAREST)
        .set_name("repeat_nearest_sampler");
    ...
    temporary_name_buffer.clear();
    cstring dither_texture_path =
        temporary_name_buffer.append_use_f("%s/BayerDither4x4.png", HYDRA_DATA_FOLDER);
    ...
    temporary_name_buffer.clear();
    cstring blue_noise_texture_path =
        temporary_name_buffer.append_use_f("%s/LDR_RG01_0.png", HYDRA_DATA_FOLDER);
    blue_noise_128_rg_texture =
        render_resources_loader.load_texture(blue_noise_texture_path, false);
  ...
  // NOTE(marco): build AS before preparing draws
  {
    ...
    as_info.flags = VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_UPDATE_BIT_KHR |
    ...
    gpu.vkGetAccelerationStructureBuildSizesKHR(gpu.vulkan_device,
                                                VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
                                                &as_info, max_primitives_count.data, &as_size_info);

    BufferCreation as_buffer_creation{};
    as_buffer_creation
        .set(VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR, ResourceUsageType::Immutable,
    ...
    as_buffer_creation.reset()
        .set(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT_KHR,
             ResourceUsageType::Immutable, as_size_info.buildScratchSize)
        .set_device_only(true)
    ...
    gpu.vkCreateAccelerationStructureKHR(gpu.vulkan_device, &as_create_info,
                                         gpu.vulkan_allocation_callbacks, &scene->blas);
    ...
    VkAccelerationStructureBuildRangeInfoKHR* blas_ranges[] = {scene->build_range_infos.data};

    gpu.vkCmdBuildAccelerationStructuresKHR(gpu_commands->vk_command_buffer, 1, &as_info,
                                            blas_ranges);
    ...
    VkDeviceAddress blas_address =
        gpu.vkGetAccelerationStructureDeviceAddressKHR(gpu.vulkan_device, &blas_address_info);
    ...
    as_buffer_creation.reset()
        .set(VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR |
                 VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
             ResourceUsageType::Immutable, sizeof(VkAccelerationStructureInstanceKHR))
    ...
    tlas_geometry.geometry.instances.data.deviceAddress =
        gpu.get_buffer_device_address(tlas_instance_buffer_handle);
    ...
    gpu.vkGetAccelerationStructureBuildSizesKHR(gpu.vulkan_device,
                                                VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
                                                &as_info, &max_instance_count, &as_size_info);
    ...
    gpu.vkCreateAccelerationStructureKHR(gpu.vulkan_device, &as_create_info,
                                         gpu.vulkan_allocation_callbacks, &scene->tlas);
    ...
    VkAccelerationStructureBuildRangeInfoKHR* tlas_ranges[] = {&tlas_range_info};
    ...
    vkCmdResetQueryPool(gpu_commands->vk_command_buffer, thread_pools->vulkan_timestamp_query_pool,
                        0, thread_pools->time_queries->time_queries.size);

    vkCmdResetQueryPool(gpu_commands->vk_command_buffer,
                        thread_pools->vulkan_pipeline_stats_query_pool, 0,
                        GpuPipelineStatistics::Count);

    vkCmdBeginQuery(gpu_commands->vk_command_buffer, thread_pools->vulkan_pipeline_stats_query_pool,
                    0, 0);
  ...
  while (!window.requested_exit) {
    ...
    if (scene->taa_enabled && scene->taa_jittering_enabled) {
      game_camera.apply_jittering(jitter_offsets.x / gpu.swapchain_width,
                                  jitter_offsets.y / gpu.swapchain_height);
    } else {
...

    {
      ...
      if (ImGui::Begin("Hydra ImGui")) {
        ...
        if (ImGui::CollapsingHeader("Lights")) {
          ...
          ImGui::SliderFloat3("Light position", selected_light.world_position.raw, -10.f, 10.f,
                              "%2.3f");
          ...
          f32 light_color[3] = {selected_light.color.x, selected_light.color.y,
                                selected_light.color.z};
        ...
        if (ImGui::CollapsingHeader("Meshlets")) {
          ...
          ImGui::Checkbox("Use meshlets cubemap face cull for shadows",
                          &shadow_meshlets_cubemap_face_cull);
        ...
        if (ImGui::CollapsingHeader("Temporal Anti-Aliasing")) {
          ...
          ImGui::Combo("Jitter Type", &current_jitter_type, JitterType::names,
                       ArraySize(JitterType::names));
          ...
          ImGui::Combo("Velocity sampling modes", &scene->taa_velocity_sampling_mode,
                       taa_velocity_mode_names, ArraySize(taa_velocity_mode_names));

          static cstring taa_history_sampling_names[] = {"None", "CatmullRom"};
          ImGui::Combo("History sampling filter", &scene->taa_history_sampling_filter,
                       taa_history_sampling_names, ArraySize(taa_history_sampling_names));

          static cstring taa_history_constraint_names[] = {"None", "Clamp", "Clip", "Variance Clip",
                                                           "Variance Clip with Color Clamping"};
          ImGui::Combo("History constraint mode", &scene->taa_history_constraint_mode,
                       taa_history_constraint_names, ArraySize(taa_history_constraint_names));

          static cstring taa_current_color_filter_names[] = {"None", "Mitchell-Netravali",
                                                             "Blackman-Harris", "Catmull-Rom"};
          ImGui::Combo("Current color filter", &scene->taa_current_color_filter,
                       taa_current_color_filter_names, ArraySize(taa_current_color_filter_names));

          ImGui::Checkbox("Inverse Luminance Filtering",
                          &scene->taa_use_inverse_luminance_filtering);
          ImGui::Checkbox("Temporal Filtering", &scene->taa_use_temporal_filtering);
          ImGui::Checkbox("Luminance Difference Filtering",
                          &scene->taa_use_luminance_difference_filtering);
          ImGui::Checkbox("Use YCoCg color space", &scene->taa_use_ycocg);
        }
        if (ImGui::CollapsingHeader("Post-Process")) {
          static cstring tonemap_names[] = {"None", "ACES"};
          ImGui::Combo("Tonemap", &scene->post_tonemap_mode, tonemap_names,
                       ArraySize(tonemap_names));
        ...
        if (ImGui::CollapsingHeader("Raytraced Shadows")) {
          ...
          ImGui::Combo("RT Light Type", &raytraced_shadow_light_type, light_type,
                       ArraySize(light_type));

          ImGui::SliderFloat("RT Light intensity", &raytraced_shadow_light_intensity, 0.01f, 10.f,
                             "%2.2f");
          ...
          ImGui::SliderFloat3("RT Light Position", raytraced_shadow_light_position.raw, -10.f, 10.f,
                              "%2.2f");
          if (raytraced_shadow_light_type == 1) {
            ImGui::EndDisabled();
          }
          ...
          ImGui::SliderFloat3("RT Directional Direction", raytraced_shadow_light_direction.raw,
                              -1.f, 1.f, "%2.2f");
          if (raytraced_shadow_light_type == 0) {
            ImGui::EndDisabled();
          }
        ...
        if (ImGui::CollapsingHeader("Global Illumination")) {
          ImGui::Text("Total Rays: %u, Rays per probe %u, Total Probes %u",
                      frame_renderer.indirect_pass.get_total_rays(),
                      frame_renderer.indirect_pass.probe_rays,
                      frame_renderer.indirect_pass.get_total_probes());
          ImGui::SliderInt("Per frame probe updates", &scene->gi_per_frame_probes_update, 0,
                           frame_renderer.indirect_pass.get_total_probes());
          ...
          if (ImGui::SliderFloat3("Probe Grid Position", scene->gi_probe_grid_position.raw, -5.f,
                                  5.f, "%2.3f")) {
            scene->gi_recalculate_offsets = true;
          }

          ImGui::Checkbox("Use Infinite Bounces", &scene->gi_use_infinite_bounces);
          ImGui::SliderFloat("Infinite bounces multiplier", &scene->gi_infinite_bounces_multiplier,
                             0.0f, 1.0f);

          if (ImGui::SliderFloat3("Probe Spacing", scene->gi_probe_spacing.raw, -2.f, 2.f,
                                  "%2.3f")) {
            scene->gi_recalculate_offsets = true;
          }
          ...
          if (ImGui::Checkbox("Use Half Resolution Output", &scene->gi_use_half_resolution)) {
            ...
            frame_renderer.indirect_pass.on_resize(gpu, &frame_graph, gpu.swapchain_width,
        ...
        if (ImGui::CollapsingHeader("Raytraced Reflections")) {
          ImGui::SliderFloat("Temporal Depth Difference", &scene->rt_temporal_depth_difference,
                             0.0f, 20.0f);
          ImGui::SliderFloat("Temporal Normal Difference", &scene->rt_temporal_normal_difference,
                             0.0f, 20.0f);
        ...
        if (ImGui::Combo("Present Mode", &present_mode, hydra::PresentMode::s_value_names,
                         hydra::PresentMode::Count)) {
          renderer.set_presentation_mode((hydra::PresentMode::Enum)present_mode);
        }
      ...
      if (ImGui::Begin("Scene")) {
        ...
        for (u32 n = 0; n < scene_graph.nodes_hierarchy.size; ++n) {
          ...
          if (ImGui::Selectable(node_debug_data.name ? node_debug_data.name : "-",
                                n == selected_node)) {
            selected_node = n;
          }
        }
      ...
      if (ImGui::Begin("Frame Graph Debug")) {
        ...
        for (u32 t = 0; t < max_textures; ++t) {
          ...
          if (texture != nullptr && texture->name != nullptr) {
            texture_names[active_texture_count] =
                texture_names_pool.append_use_f("%s (%d)", texture->name, t);
      ...
      if (ImGui::Begin("Lights Debug")) {
        ...
        for (u32 l = 0; l < lights_count; ++l) {
          ...
          ImGui::Text("%d: %d, %d R: %0.2f a: %0.6f", l, light.tile_x, light.tile_y,
                      light.shadow_map_resolution, light.solid_angle);
        }
      }
      ImGui::End();
    }
...

    {
      ...
      if (scene->taa_jittering_enabled && scene->taa_enabled) {
        scene_data.jitter_xy = vec2s{(scene_data.halton_x) / gpu.swapchain_width,
                                     (scene_data.halton_y) / gpu.swapchain_height};
      }
      ...
      scene_data.camera_position =
          vec4s{game_camera.camera.position.x, game_camera.camera.position.y,
                game_camera.camera.position.z, 1.0f};
      ...
      scene_data.volumetric_fog_application_dithering_scale =
          scene->volumetric_fog_application_dithering_scale;
      scene_data.volumetric_fog_application_options =
          (scene->volumetric_fog_application_apply_opacity_anti_aliasing ? 1 : 0) |
      ...
      scene_data.frustum_planes[0] = normalize_plane(
          glms_vec4_add(projection_transpose.col[3], projection_transpose.col[0]));  // x + w  < 0;
      scene_data.frustum_planes[1] = normalize_plane(
          glms_vec4_sub(projection_transpose.col[3], projection_transpose.col[0]));  // x - w  < 0;
      scene_data.frustum_planes[2] = normalize_plane(
          glms_vec4_add(projection_transpose.col[3], projection_transpose.col[1]));  // y + w  < 0;
      scene_data.frustum_planes[3] = normalize_plane(
          glms_vec4_sub(projection_transpose.col[3], projection_transpose.col[1]));  // y - w  < 0;
      scene_data.frustum_planes[4] = normalize_plane(
          glms_vec4_add(projection_transpose.col[3], projection_transpose.col[2]));  // z + w  < 0;
      scene_data.frustum_planes[5] = normalize_plane(
          glms_vec4_sub(projection_transpose.col[3], projection_transpose.col[2]));  // z - w  < 0;
      ...
      if (gpu_lighting_data) {
        ...
        if (resource) {
          gpu_lighting_data->shadow_visibility_texture_index =
              resource->resource_info.texture.handle.index;
        }

        resource = (FrameGraphResource*)frame_graph.get_resource("indirect_lighting");
        if (resource) {
          gpu_lighting_data->indirect_lighting_texture_index =
              resource->resource_info.texture.handle.index;
        }

        resource = (FrameGraphResource*)frame_graph.get_resource("bilateral_weights");
        if (resource) {
          gpu_lighting_data->bilateral_weights_texture_index =
              resource->resource_info.texture.handle.index;
        }

        resource = (FrameGraphResource*)frame_graph.get_resource("svgf_output");
        if (resource) {
          gpu_lighting_data->reflections_texture_index =
              resource->resource_info.texture.handle.index;
        }
        ...
        const float one_over_log_f_over_n =
            1.0f / log2(game_camera.camera.far_plane / game_camera.camera.near_plane);
        gpu_lighting_data->volumetric_fog_distribution_scale =
            scene->volumetric_fog_slices * one_over_log_f_over_n;
        gpu_lighting_data->volumetric_fog_distribution_bias =
            -(scene->volumetric_fog_slices * log2(game_camera.camera.near_plane) *
              one_over_log_f_over_n);

        hydra::Color raytraced_light_color_type_packed;
        raytraced_light_color_type_packed.set(
            raytraced_shadow_light_color.x, raytraced_shadow_light_color.y,
            raytraced_shadow_light_color.z, raytraced_shadow_light_type);
        gpu_lighting_data->raytraced_shadow_light_color_type =
            raytraced_light_color_type_packed.abgr;
        gpu_lighting_data->raytraced_shadow_light_radius = raytraced_shadow_light_radius;
        gpu_lighting_data->raytraced_shadow_light_position = raytraced_shadow_light_type == 0
                                                                 ? raytraced_shadow_light_position
                                                                 : raytraced_shadow_light_direction;
      ...
      if ((input.is_mouse_clicked(MOUSE_BUTTONS_LEFT) ||
           input.is_mouse_dragging(MOUSE_BUTTONS_LEFT)) &&
      ...
      if (scene->show_light_edit_debug_draws) {
        ...
        scene->debug_renderer.aabb(
            glms_vec3_sub(light.world_position, {half_radius, half_radius, half_radius}),
            glms_vec3_add(light.world_position, {half_radius, half_radius, half_radius}),
            {Color::white});
        scene->debug_renderer.aabb(glms_vec3_sub(light.world_position, {.1, .1, .1}),
                                   glms_vec3_add(light.world_position, {.1, .1, .1}),
                                   {Color::green});
      }
    }
    ...
    if (!window.minimized) {
      DrawTask draw_task;
      draw_task.init(renderer.gpu, &frame_graph, &renderer, imgui, &gpu_profiler, scene,
                     &frame_renderer);
      ...
      CommandBuffer* async_compute_command_buffer = nullptr;
      {
        ...
        async_compute_command_buffer =
            scene->update_physics(delta_time, air_density, spring_stiffness, spring_damping,
                                  wind_direction, reset_simulation);
  ...
  gpu.vkDestroyAccelerationStructureKHR(gpu.vulkan_device, scene->blas,
                                        gpu.vulkan_allocation_callbacks);
  gpu.destroy_buffer(scene->tlas_buffer);
  gpu.vkDestroyAccelerationStructureKHR(gpu.vulkan_device, scene->tlas,
                                        gpu.vulkan_allocation_callbacks);
