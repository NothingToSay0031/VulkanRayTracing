...
// Graphics Data
static hydra::TextureHandle g_font_texture;
static hydra::PipelineHandle g_imgui_pipeline;
static hydra::BufferHandle g_vb, g_ib;
static hydra::BufferHandle g_ui_cb;
static hydra::DescriptorSetLayoutHandle g_descriptor_set_layout;
static hydra::DescriptorSetHandle g_ui_descriptor_set;  // Font descriptor set

static uint32_t g_vb_size = 665536, g_ib_size = 665536;

hydra::FlatHashMap<hydra::ResourceHandle, hydra::ResourceHandle> g_texture_to_descriptor_set;
...
ImGuiService* ImGuiService::instance() { return &s_imgui_service; }
...
void ImGuiService::init(void* configuration) {
  ...
  if (gpu->bindless_supported) {
    shader_creation.set_name("ImGui")
        .add_stage(g_vertex_shader_code_bindless, (uint32_t)strlen(g_vertex_shader_code_bindless),
                   VK_SHADER_STAGE_VERTEX_BIT)
        .add_stage(g_fragment_shader_code_bindless,
                   (uint32_t)strlen(g_fragment_shader_code_bindless), VK_SHADER_STAGE_FRAGMENT_BIT);
  } else {
    shader_creation.set_name("ImGui")
        .add_stage(g_vertex_shader_code, (uint32_t)strlen(g_vertex_shader_code),
                   VK_SHADER_STAGE_VERTEX_BIT)
        .add_stage(g_fragment_shader_code, (uint32_t)strlen(g_fragment_shader_code),
  ...
  pipeline_creation.blend_state.add_blend_state().set_color(
  ...
  if (!gpu->bindless_supported) {
    descriptor_set_layout_creation.add_binding(
  ...
  cb_creation.set(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, ResourceUsageType::Dynamic, 64)
      .set_name("CB_ImGui");
  ...
  vb_creation.set(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, ResourceUsageType::Dynamic, g_vb_size)
      .set_name("VB_ImGui");
  g_vb = gpu->create_buffer(vb_creation);

  BufferCreation ib_creation;
  ib_creation.set(VK_BUFFER_USAGE_INDEX_BUFFER_BIT, ResourceUsageType::Dynamic, g_ib_size)
      .set_name("IB_ImGui");
...
void ImGuiService::shutdown() {
  ...
  while (it.is_valid()) {
    hydra::ResourceHandle handle = g_texture_to_descriptor_set.get(it);
...
void ImGuiService::render(hydra::CommandBuffer& commands, bool use_secondary) {
  ImGui::Render();
        ...
        if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f &&
          ...
          if (!gpu->bindless_supported) {
            if (new_texture.index != last_texture.index &&
                new_texture.index != k_invalid_texture.index) {
              ...
              if (it.is_invalid()) {
                ...
                ds_creation.set_layout(g_descriptor_set_layout)
                    .buffer(g_ui_cb, 0)
                    .texture(last_texture, 1)
                    .set_name("RL_Dynamic_ImGUI");
          ...
          commands.draw_indexed(hydra::TopologyType::Triangle, pcmd->ElemCount, 1,
...
void ImGuiService::remove_cached_texture(hydra::TextureHandle& texture) {
  FlatHashMapIterator it = g_texture_to_descriptor_set.find(texture.index);
  if (it.is_valid()) {
    // Destroy descriptor set
    hydra::DescriptorSetHandle descriptor_set{g_texture_to_descriptor_set.get(it)};
...
bool imgui_file_dialog_open( const char* button_name, const char* path, const char* extension ) {
    ...
    if ( opened && ImGui::Begin( "hydra_imgui_file_dialog", &opened,
...
struct ExampleAppLog {
  ...
  void Draw(const char* title, bool* p_open = NULL) {
    ...
    if (Filter.IsActive()) {
      ...
      // recommended. Using ImGuiListClipper requires A) random access into your data, and B) items
      // all being the  same height, both of which we can handle since we an array pointing to the
      // beginning of each line of text. When using the filter (in the block of code above) we don't
      // have random access into the data to display anymore, which is why we don't use the clipper.
...
static void imgui_print(const char* text) { s_imgui_log.AddLog("%s", text); }

void imgui_log_init() { LogService::instance()->set_callback(&imgui_print); }

void imgui_log_shutdown() { LogService::instance()->set_callback(nullptr); }

void imgui_log_draw() { s_imgui_log.Draw("Log", &s_imgui_log_open); }
...
class Sparkline {
 public:
  ...
  void draw(char const* const label = "", ImVec2 const size = ImVec2()) const {
    ...
    ImGui::PlotLines(label, getValue, const_cast<Sparkline*>(this), L, 0, overlay, _min, _max,
                     size);
  }
...
void imgui_fps_add(f32 dt) { s_fps_line.add(dt); }

void imgui_fps_draw() { s_fps_line.draw("FPS", {0, 100}); }
}  // namespace hydra
...
bool SliderUint(const char* label, u32* v, u32 v_min, u32 v_max, const char* format,
                ImGuiSliderFlags flags) {
  return SliderScalar(label, ImGuiDataType_U32, v, &v_min, &v_max, format, flags);
}

bool SliderUint2(const char* label, u32 v[2], u32 v_min, u32 v_max, const char* format,
                 ImGuiSliderFlags flags) {
  return SliderScalarN(label, ImGuiDataType_U32, v, 2, &v_min, &v_max, format, flags);
}

bool SliderUint3(const char* label, u32 v[3], u32 v_min, u32 v_max, const char* format,
                 ImGuiSliderFlags flags) {
  return SliderScalarN(label, ImGuiDataType_U32, v, 3, &v_min, &v_max, format, flags);
}

bool SliderUint4(const char* label, u32 v[4], u32 v_min, u32 v_max, const char* format,
                 ImGuiSliderFlags flags) {
