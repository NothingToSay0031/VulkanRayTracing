...
void GpuVisualProfiler::init(Allocator* allocator_, u32 max_frames_, u32 max_queries_per_frame_) {
  ...
  timestamps =
      (GPUTimeQuery*)ralloca(sizeof(GPUTimeQuery) * max_frames * max_queries_per_frame, allocator);
...
void GpuVisualProfiler::update(GpuDevice& gpu) {
  ...
  if (paused && !gpu.resized) return;

  // Collect timestamps
  u32 active_timestamps =
      gpu.copy_gpu_timestamps(&timestamps[max_queries_per_frame * current_frame]);
  ...
  for (u32 i = 0; i < active_timestamps; ++i) {
    ...
    const u64 hashed_name = hydra::hash_calculate(timestamp.name);
    ...
    timestamp.color = hydra::Color::get_distinct_color(color_index);
  }
...
void GpuVisualProfiler::imgui_draw() {
...

  {
    ...
    draw_list->AddLine({cursor_pos.x + rect_width, cursor_pos.y},
                       {cursor_pos.x + graph_width, cursor_pos.y}, 0xff0000ff);
    ...
    for (u32 i = 0; i < max_frames; ++i) {
      ...
      frame_time = hydra::clamp(frame_time, 0.00001f, 1000.f);
      // Update timings
      ...
      for (u32 j = 0; j < per_frame_active[frame_index]; ++j) {
        ...
        const ImVec2 rect_max{frame_x + width_margin + rect_width - width_margin,
      ...
      draw_list->AddLine({frame_x, cursor_pos.y + widget_height}, {frame_x, cursor_pos.y},
                         0x0fffffff);
    ...
    if (selected_frame >= 0) {
      ...
      for (u32 j = 0; j < per_frame_active[selected_frame]; ++j) {
        ...
        if (timestamp.depth == 0) {
          ...
          draw_list->AddRectFilled({timestamp_x, y + 4}, {timestamp_x + 8, y + 12},
                                   timestamp.color);
  ...
  if (pipeline_statistics) {
    ...
    ImGui::Text("Clipping: Invocations %0.2f%s, Visible Primitives %0.2f%s, Visible Perc %3.1f",
                ...
                stat_values[GpuPipelineStatistics::ClippingPrimitives] /
                    stat_values[GpuPipelineStatistics::ClippingInvocations] * 100.0f,
                stat_unit_name);

    ImGui::Text(
        "Invocations: Vertex Shaders %0.2f%s, Fragment Shaders %0.2f%s, Compute Shaders %0.2f%s",
    ...
    ImGui::Text("Vertex %0.2f, Fragment %0.2f, Compute %0.2f",
                stat_values[GpuPipelineStatistics::VertexShaderInvocations] * stat_unit_multiplier /
                    s_framebuffer_pixel_count,
                stat_values[GpuPipelineStatistics::FragmentShaderInvocations] *
                    stat_unit_multiplier / s_framebuffer_pixel_count,
                stat_values[GpuPipelineStatistics::ComputeShaderInvocations] *
                    stat_unit_multiplier / s_framebuffer_pixel_count);
  }
...
void GPUTimeQueriesManager::init(GpuThreadFramePools* thread_frame_pools_, Allocator* allocator_,
...
void GPUTimeQueriesManager::reset() { current_frame_resolved = false; }
...
u32 GPUTimeQueriesManager::resolve(u32 current_frame, GPUTimeQuery* timestamps_to_fill) {
  ...
  for (u32 t = 0; t < num_threads; ++t) {
    ...
    if (time_query && time_query->allocated_time_query) {
      hydra::memory_copy(timestamps_to_fill + copied_timestamps,
                          &timestamps[pool_index * queries_per_thread],
