...
// BlendState  ////////////////////////////////////////////////////////////
BlendState& BlendState::set_color(VkBlendFactor source, VkBlendFactor destination,
                                  VkBlendOp operation) {
  source_color = source;
...
BlendState& BlendState::set_alpha(VkBlendFactor source, VkBlendFactor destination,
                                  VkBlendOp operation) {
  source_alpha = source;
...
BlendState& BlendStateCreation::add_blend_state() { return blend_states[active_states++]; }
...
BufferCreation& BufferCreation::set(VkBufferUsageFlags flags, ResourceUsageType::Enum usage_,
                                    u32 size_) {
  type_flags = flags;
  usage = usage_;
  size = size_;
...
// SamplerCreation ////////////////////////////////////////////////////////
SamplerCreation& SamplerCreation::set_min_mag_mip(VkFilter min, VkFilter mag,
...
SamplerCreation& SamplerCreation::set_address_mode_uv(VkSamplerAddressMode u,
...
SamplerCreation& SamplerCreation::set_address_mode_uvw(VkSamplerAddressMode u,
                                                       VkSamplerAddressMode v,
...
ShaderStateCreation& ShaderStateCreation::add_stage(const char* code, sizet code_size,
...
DescriptorSetLayoutCreation& DescriptorSetLayoutCreation::add_binding(VkDescriptorType type,
                                                                      u32 index, u32 count,
                                                                      cstring name) {
...
DescriptorSetCreation& DescriptorSetCreation::texture_sampler(TextureHandle texture,
                                                              SamplerHandle sampler, u16 binding) {
  bindings[num_resources] = binding;
  resources[num_resources] = texture.index;
...
RenderPassOutput& RenderPassOutput::color(VkFormat format, VkImageLayout layout,
...
RenderPassOutput& RenderPassOutput::set_depth_stencil_operations(
    RenderPassOperation::Enum depth_, RenderPassOperation::Enum stencil_) {
...
RenderPassOutput& PipelineCreation::render_pass_output() { return render_pass; }
...
RenderPassCreation& RenderPassCreation::add_attachment(VkFormat format, VkImageLayout layout,
                                                       RenderPassOperation::Enum load_op) {
...
RenderPassCreation& RenderPassCreation::set_depth_stencil_texture(VkFormat format,
...
RenderPassCreation& RenderPassCreation::set_depth_stencil_operations(
    RenderPassOperation::Enum depth_, RenderPassOperation::Enum stencil_) {
  depth_operation = depth_;
  stencil_operation = stencil_;
...
//
// Texture1D, Texture2D, Texture3D, TextureCube, Texture_1D_Array, Texture_2D_Array,
...
VkImageViewType to_vk_image_view_type(TextureType::Enum type) {
  static VkImageViewType s_vk_data[] = {VK_IMAGE_VIEW_TYPE_1D,        VK_IMAGE_VIEW_TYPE_2D,
...
VkImageLayout util_to_vk_image_layout2(ResourceState usage) {
  ...
  if (usage & RESOURCE_STATE_RENDER_TARGET) return VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL_KHR;

  if (usage & RESOURCE_STATE_DEPTH_WRITE) return VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL_KHR;

  if (usage & RESOURCE_STATE_DEPTH_READ) return VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL_KHR;

  if (usage & RESOURCE_STATE_UNORDERED_ACCESS) return VK_IMAGE_LAYOUT_GENERAL;

  if (usage & RESOURCE_STATE_SHADER_RESOURCE) return VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL_KHR;
...
VkPipelineStageFlags util_determine_pipeline_stage_flags(VkAccessFlags access_flags,
  ...
  switch (queue_type) {
    case QueueType::Graphics: {
      ...
      if ((access_flags & (VK_ACCESS_ACCELERATION_STRUCTURE_READ_BIT_KHR |
...
VkPipelineStageFlags2KHR util_determine_pipeline_stage_flags2(VkAccessFlags2KHR access_flags,
                                                              QueueType::Enum queue_type) {
  ...
  switch (queue_type) {
    case QueueType::Graphics: {
      ...
      if ((access_flags & (VK_ACCESS_2_ACCELERATION_STRUCTURE_READ_BIT_KHR |
  ...
  if (flags == 0) flags = VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT_KHR;

  return flags;
}

void util_add_image_barrier(GpuDevice* gpu, VkCommandBuffer command_buffer, VkImage image,
                            ResourceState old_state, ResourceState new_state, u32 base_mip_level,
                            u32 mip_count, bool is_depth) {
  if (gpu->synchronization2_extension_present) {
    ...
    barrier.srcStageMask =
        util_determine_pipeline_stage_flags2(barrier.srcAccessMask, QueueType::Graphics);
    ...
    const VkPipelineStageFlags source_stage_mask =
        util_determine_pipeline_stage_flags(barrier.srcAccessMask, QueueType::Graphics);
    const VkPipelineStageFlags destination_stage_mask =
        util_determine_pipeline_stage_flags(barrier.dstAccessMask, QueueType::Graphics);

    vkCmdPipelineBarrier(command_buffer, source_stage_mask, destination_stage_mask, 0, 0, nullptr,
                         0, nullptr, 1, &barrier);
  }
}

void util_add_image_barrier(GpuDevice* gpu, VkCommandBuffer command_buffer, Texture* texture,
                            ResourceState new_state, u32 base_mip_level, u32 mip_count,
                            bool is_depth) {
  // rprint( "Transitioning Texture %s from %s to %s\n", texture->name, ResourceStateName(
  // texture->state ), ResourceStateName( new_state ) );
  if (strcmp("motion_vectors", texture->name) == 0) {
    // rprint( "Transitioning Texture %s from %s to %s\n", texture->name, ResourceStateName(
    // texture->state ), ResourceStateName( new_state ) );
  }
  util_add_image_barrier(gpu, command_buffer, texture->vk_image, texture->state, new_state,
                         base_mip_level, mip_count, is_depth);
  texture->state = new_state;
}

void util_add_image_barrier_ext(GpuDevice* gpu, VkCommandBuffer command_buffer, VkImage image,
                                ResourceState old_state, ResourceState new_state,
                                u32 base_mip_level, u32 mip_count, u32 base_array_layer,
                                u32 array_layer_count, bool is_depth, u32 source_family,
                                u32 destination_family, QueueType::Enum source_queue_type,
                                QueueType::Enum destination_queue_type) {
  if (gpu->synchronization2_extension_present) {
    VkImageMemoryBarrier2KHR barrier{VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2_KHR};
    barrier.srcAccessMask = util_to_vk_access_flags2(old_state);
    barrier.srcStageMask =
        util_determine_pipeline_stage_flags2(barrier.srcAccessMask, source_queue_type);
    barrier.dstAccessMask = util_to_vk_access_flags2(new_state);
    barrier.dstStageMask =
        util_determine_pipeline_stage_flags2(barrier.dstAccessMask, destination_queue_type);
    barrier.oldLayout = util_to_vk_image_layout2(old_state);
    barrier.newLayout = util_to_vk_image_layout2(new_state);
    ...
    const VkPipelineStageFlags source_stage_mask =
        util_determine_pipeline_stage_flags(barrier.srcAccessMask, source_queue_type);
    const VkPipelineStageFlags destination_stage_mask =
        util_determine_pipeline_stage_flags(barrier.dstAccessMask, destination_queue_type);
...
void util_add_image_barrier_ext(GpuDevice* gpu, VkCommandBuffer command_buffer, Texture* texture,
                                ResourceState new_state, u32 base_mip_level, u32 mip_count,
                                u32 base_array_layer, u32 array_layer_count, bool is_depth,
                                u32 source_family, u32 destination_family,
                                QueueType::Enum source_queue_type,
                                QueueType::Enum destination_queue_type) {
  util_add_image_barrier_ext(gpu, command_buffer, texture->vk_image, texture->state, new_state,
                             base_mip_level, mip_count, base_array_layer, array_layer_count,
                             is_depth, source_family, destination_family, source_queue_type,
                             destination_queue_type);
  texture->state = new_state;
}

void util_add_buffer_barrier(GpuDevice* gpu, VkCommandBuffer command_buffer, VkBuffer buffer,
                             ResourceState old_state, ResourceState new_state, u32 buffer_size) {
  util_add_buffer_barrier_ext(gpu, command_buffer, buffer, old_state, new_state, buffer_size,
                              VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED, QueueType::Graphics,
                              QueueType::Graphics);
}

void util_add_buffer_barrier_ext(GpuDevice* gpu, VkCommandBuffer command_buffer, VkBuffer buffer,
                                 ResourceState old_state, ResourceState new_state, u32 buffer_size,
                                 u32 source_family, u32 destination_family,
                                 QueueType::Enum source_queue_type,
                                 QueueType::Enum destination_queue_type) {
