...
void CommandBuffer::init(GpuDevice* gpu) {
  ...
  VkResult result =
      vkCreateDescriptorPool(gpu_device->vulkan_device, &pool_info,
                             gpu_device->vulkan_allocation_callbacks, &vk_descriptor_pool);
...
void CommandBuffer::shutdown() {
  ...
  vkDestroyDescriptorPool(gpu_device->vulkan_device, vk_descriptor_pool,
...
void CommandBuffer::begin_secondary(RenderPass* current_render_pass_,
                                    Framebuffer* current_framebuffer_) {
  if (!is_recording) {
...
void CommandBuffer::bind_pass(RenderPassHandle handle_, FramebufferHandle framebuffer_,
                              bool use_secondary) {
  // if ( !is_recording )
  {
    is_recording = true;
    ...
    if (render_pass != current_render_pass) {
      if (gpu_device->dynamic_rendering_extension_present) {
        ...
        color_attachments_info.init(gpu_device->temporary_allocator,
                                    framebuffer->num_color_attachments,
                                    framebuffer->num_color_attachments);
        memset(color_attachments_info.data, 0,
               sizeof(VkRenderingAttachmentInfoKHR) * framebuffer->num_color_attachments);
        ...
        for (u32 a = 0; a < framebuffer->num_color_attachments; ++a) {
          ...
          color_attachment_info.imageLayout = gpu_device->synchronization2_extension_present
          ...
          color_attachment_info.clearValue =
              render_pass->output.color_operations[a] == RenderPassOperation::Enum::Clear
                  ? clear_values[a]
                  : VkClearValue{};
        }
        ...
        if (has_depth_attachment) {
          ...
          depth_attachment_info.imageLayout =
              gpu_device->synchronization2_extension_present
                  ? VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL_KHR
          ...
          depth_attachment_info.clearValue =
              render_pass->output.depth_operation == RenderPassOperation::Enum::Clear
                  ? clear_values[k_depth_stencil_clear_index]
                  : VkClearValue{};
        }

        VkRenderingInfoKHR rendering_info{VK_STRUCTURE_TYPE_RENDERING_INFO_KHR};
        rendering_info.flags =
            use_secondary ? VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT_KHR : 0;
        ...
        rendering_info.pColorAttachments =
            framebuffer->num_color_attachments > 0 ? color_attachments_info.data : nullptr;
        ...
        if (framebuffer->shader_rate_attachment.index != k_invalid_index) {
          ...
          shading_rate_info.shadingRateAttachmentTexelSize =
              gpu_device->min_fragment_shading_rate_texel_size;
        ...
        vkCmdBeginRenderPass(vk_command_buffer, &render_pass_begin,
                             use_secondary ? VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS
...
void CommandBuffer::bind_vertex_buffers(BufferHandle* handles, u32 first_binding, u32 binding_count,
                                        u32* offsets_) {
...
void CommandBuffer::bind_descriptor_set(DescriptorSetHandle* handles, u32 num_lists, u32* offsets,
                                        u32 num_offsets) {
  // TODO:
  u32 offsets_cache[8];
  num_offsets = 0;
  ...
  vkCmdBindDescriptorSets(vk_command_buffer, current_pipeline->vk_bind_point,
                          current_pipeline->vk_pipeline_layout, k_first_set, num_lists,
                          vk_descriptor_sets, num_offsets, offsets_cache);

  if (gpu_device->bindless_supported) {
    vkCmdBindDescriptorSets(vk_command_buffer, current_pipeline->vk_bind_point,
                            current_pipeline->vk_pipeline_layout, 0, 1,
                            &gpu_device->vulkan_bindless_descriptor_set_cached, 0, nullptr);
  }
}

void CommandBuffer::bind_local_descriptor_set(DescriptorSetHandle* handles, u32 num_lists,
                                              u32* offsets, u32 num_offsets) {
  // TODO:
  u32 offsets_cache[8];
  num_offsets = 0;
  ...
  for (u32 l = 0; l < num_lists; ++l) {
    DescriptorSet* descriptor_set =
        (DescriptorSet*)descriptor_sets.access_resource(handles[l].index);
    vk_descriptor_sets[l] = descriptor_set->vk_descriptor_set;
...
void CommandBuffer::push_constants(PipelineHandle pipeline, u32 offset, u32 size, void* data) {
  ...
  vkCmdPushConstants(vk_command_buffer, pipeline_->vk_pipeline_layout, VK_SHADER_STAGE_ALL, offset,
                     size, data);
}

void CommandBuffer::draw(TopologyType::Enum topology, u32 first_vertex, u32 vertex_count,
                         u32 first_instance, u32 instance_count) {
  vkCmdDraw(vk_command_buffer, vertex_count, instance_count, first_vertex, first_instance);
}

void CommandBuffer::draw_indexed(TopologyType::Enum topology, u32 index_count, u32 instance_count,
                                 u32 first_index, i32 vertex_offset, u32 first_instance) {
  vkCmdDrawIndexed(vk_command_buffer, index_count, instance_count, first_index, vertex_offset,
                   first_instance);
}
...
void CommandBuffer::draw_indirect(BufferHandle buffer_handle, u32 draw_count, u32 offset,
                                  u32 stride) {
  Buffer* buffer = gpu_device->access_buffer(buffer_handle);
...
void CommandBuffer::draw_indirect_count(BufferHandle argument_buffer, u32 argument_offset,
  ...
  vkCmdDrawIndirectCount(vk_command_buffer, argument_buffer_->vk_buffer, argument_offset,
                         count_buffer_->vk_buffer, count_offset, max_draws, stride);
}

void CommandBuffer::draw_indexed_indirect(BufferHandle buffer_handle, u32 draw_count, u32 offset,
                                          u32 stride) {
  Buffer* buffer = gpu_device->access_buffer(buffer_handle);
...
void CommandBuffer::draw_mesh_task_indirect(BufferHandle argument_buffer, u32 argument_offset,
                                            u32 command_count, u32 stride) {
  Buffer* argument_buffer_ = gpu_device->access_buffer(argument_buffer);

  gpu_device->vkCmdDrawMeshTasksIndirectNV(vk_command_buffer, argument_buffer_->vk_buffer,
                                           argument_offset, command_count, stride);
}

void CommandBuffer::draw_mesh_task_indirect_count(BufferHandle argument_buffer, u32 argument_offset,
                                                  BufferHandle count_buffer, u32 count_offset,
                                                  u32 max_draws, u32 stride) {
  Buffer* argument_buffer_ = gpu_device->access_buffer(argument_buffer);
  Buffer* count_buffer_ = gpu_device->access_buffer(count_buffer);

  gpu_device->vkCmdDrawMeshTasksIndirectCountNV(vk_command_buffer, argument_buffer_->vk_buffer,
                                                argument_offset, count_buffer_->vk_buffer,
                                                count_offset, max_draws, stride);
}
...
void CommandBuffer::trace_rays(PipelineHandle pipeline_, u32 width, u32 height, u32 depth) {
  ...
  raygen_table.deviceAddress =
      gpu_device->get_buffer_device_address(pipeline->shader_binding_table_raygen);
  ...
  hit_table.deviceAddress =
      gpu_device->get_buffer_device_address(pipeline->shader_binding_table_hit);
  ...
  miss_table.deviceAddress =
      gpu_device->get_buffer_device_address(pipeline->shader_binding_table_miss);
  ...
  gpu_device->vkCmdTraceRaysKHR(vk_command_buffer, &raygen_table, &miss_table, &hit_table,
                                &callable_table, width, height, depth);
}
...
static ResourceState to_resource_state(PipelineStage::Enum stage) {
  static ResourceState s_states[] = {RESOURCE_STATE_INDIRECT_ARGUMENT,
...
void CommandBuffer::issue_buffer_barrier(BufferHandle buffer_handle, ResourceState old_state,
                                         ResourceState new_state, QueueType::Enum source_queue_type,
                                         QueueType::Enum destination_queue_type) {
  Buffer* buffer = gpu_device->access_buffer(buffer_handle);
  util_add_buffer_barrier_ext(gpu_device, vk_command_buffer, buffer->vk_buffer,
                              RESOURCE_STATE_UNORDERED_ACCESS, RESOURCE_STATE_UNORDERED_ACCESS,
                              buffer->size, VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED,
                              source_queue_type, destination_queue_type);
}

void CommandBuffer::issue_texture_barrier(TextureHandle texture_handle, ResourceState new_state,
                                          u32 mip_level, u32 mip_count) {
  Texture* texture = gpu_device->access_texture(texture_handle);
  util_add_image_barrier(gpu_device, vk_command_buffer, texture, new_state, mip_level, mip_count,
...
//            barrier.srcStageMask = util_determine_pipeline_stage_flags2( barrier.srcAccessMask,
//            QueueType::Graphics ); barrier.dstAccessMask = util_to_vk_access_flags2(
//            source_barrier.destination_state ); barrier.dstStageMask =
//            util_determine_pipeline_stage_flags2( barrier.dstAccessMask, QueueType::Graphics );
...
//            barrier.subresourceRange.aspectMask = TextureFormat::has_depth_or_stencil(
...
void CommandBuffer::clear_color_image(TextureHandle texture, VkClearColorValue clear_color) {
  ...
  util_add_image_barrier(gpu_device, vk_command_buffer, vk_texture,
                         ResourceState::RESOURCE_STATE_COPY_DEST, 0, VK_REMAINING_MIP_LEVELS,
                         false);

  vkCmdClearColorImage(vk_command_buffer, vk_texture->vk_image,
...
void CommandBuffer::fill_buffer(BufferHandle buffer, u32 offset, u32 size, u32 data) {
  ...
  vkCmdFillBuffer(vk_command_buffer, vk_buffer->vk_buffer, VkDeviceSize(offset),
                  size ? VkDeviceSize(size) : VkDeviceSize(vk_buffer->size), data);
}
...
void CommandBuffer::push_marker(const char* name) {
  ...
  vkCmdWriteTimestamp(vk_command_buffer, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
                      thread_frame_pool->vulkan_timestamp_query_pool,
                      time_query->start_query_index);

  if (!gpu_device->debug_utils_extension_present) return;
...
void CommandBuffer::pop_marker() {
  ...
  vkCmdWriteTimestamp(vk_command_buffer, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
                      thread_frame_pool->vulkan_timestamp_query_pool, time_query->end_query_index);
...
u32 CommandBuffer::get_subgroup_sized(u32 group) {
  return hydra::ceilu32(group * 1.f / gpu_device->subgroup_size);
}

void CommandBuffer::upload_texture_data(TextureHandle texture_handle, void* texture_data,
  ...
  memcpy(staging_buffer->mapped_data + staging_buffer_offset, texture_data,
  ...
  util_add_image_barrier(gpu_device, vk_command_buffer, texture, RESOURCE_STATE_COPY_DEST, 0, 1,
                         false);
  // Copy from the staging buffer to the image
  vkCmdCopyBufferToImage(vk_command_buffer, staging_buffer->vk_buffer, texture->vk_image,
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);

  // Post copy memory barrier
  util_add_image_barrier_ext(gpu_device, vk_command_buffer, texture, RESOURCE_STATE_COPY_SOURCE, 0,
                             1, 0, 1, false, gpu_device->vulkan_transfer_queue_family,
                             gpu_device->vulkan_main_queue_family, QueueType::CopyTransfer,
                             QueueType::Graphics);
}
...
void CommandBuffer::copy_texture(TextureHandle src_, TextureHandle dst_, ResourceState dst_state) {
  ...
  // Copy from the staging buffer to the image
  util_add_image_barrier(gpu_device, vk_command_buffer, src, RESOURCE_STATE_COPY_SOURCE, 0, 1,
                         src_is_depth);
  // TODO(marco): maybe we need a state per mip?
  ResourceState old_state = dst->state;
  util_add_image_barrier(gpu_device, vk_command_buffer, dst, RESOURCE_STATE_COPY_DEST, 0, 1,
                         dst_is_depth);

  vkCmdCopyImage(vk_command_buffer, src->vk_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
  ...
  if (dst->mip_level_count > 1) {
    ...
    util_add_image_barrier(gpu_device, vk_command_buffer, dst, RESOURCE_STATE_COPY_SOURCE, 0, 1,
                           dst_is_depth);
  }
  ...
  for (int mip_index = 1; mip_index < dst->mip_level_count; ++mip_index) {
    util_add_image_barrier(gpu_device, vk_command_buffer, dst->vk_image, old_state,
                           RESOURCE_STATE_COPY_DEST, mip_index, 1, dst_is_depth);
    ...
    vkCmdBlitImage(vk_command_buffer, dst->vk_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                   dst->vk_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit_region,
                   VK_FILTER_LINEAR);

    // Prepare current mip for next level
    util_add_image_barrier(gpu_device, vk_command_buffer, dst->vk_image, RESOURCE_STATE_COPY_DEST,
                           RESOURCE_STATE_COPY_SOURCE, mip_index, 1, dst_is_depth);
  }

  // Transition
  util_add_image_barrier(gpu_device, vk_command_buffer, dst, dst_state, 0, dst->mip_level_count,
                         dst_is_depth);
}

void CommandBuffer::copy_texture(TextureHandle src_, TextureSubResource src_sub, TextureHandle dst_,
  ...
  for (int mip_index = 1; mip_index < dst->mip_level_count; ++mip_index) {
    ...
    // Prepare current mip for next level
    util_add_image_barrier(gpu_device, vk_command_buffer, dst->vk_image, RESOURCE_STATE_COPY_DEST,
                           RESOURCE_STATE_COPY_SOURCE, mip_index, 1, false);
  }
...
void CommandBuffer::copy_buffer(BufferHandle src, sizet src_offset, BufferHandle dst,
                                sizet dst_offset, sizet size) {
...
void CommandBuffer::upload_buffer_data(BufferHandle buffer_handle, void* buffer_data,
                                       BufferHandle staging_buffer_handle,
                                       sizet staging_buffer_offset) {
  Buffer* buffer = gpu_device->access_buffer(buffer_handle);
  Buffer* staging_buffer = gpu_device->access_buffer(staging_buffer_handle);
  ...
  // Copy buffer_data to staging buffer
  memcpy(staging_buffer->mapped_data + staging_buffer_offset, buffer_data,
  ...
  util_add_buffer_barrier_ext(
      gpu_device, vk_command_buffer, buffer->vk_buffer, RESOURCE_STATE_COPY_DEST,
      RESOURCE_STATE_UNDEFINED, copy_size, gpu_device->vulkan_transfer_queue_family,
      gpu_device->vulkan_main_queue_family, QueueType::CopyTransfer, QueueType::Graphics);
}
...
void CommandBufferManager::reset_pools(u32 frame_index) {
  for (u32 i = 0; i < num_pools_per_frame; i++) {
    ...
    vkResetCommandPool(gpu->vulkan_device, gpu->thread_frame_pools[pool_index].vulkan_command_pool,
                       0);
...
CommandBuffer* CommandBufferManager::get_command_buffer(u32 frame, u32 thread_index, bool begin) {
  ...
  CommandBuffer* cb =
      &command_buffers[(pool_index * num_command_buffers_per_thread) + current_used_buffer];
  if (begin) {
    cb->reset();
    cb->begin();
    ...
    vkCmdResetQueryPool(cb->vk_command_buffer, thread_pools->vulkan_timestamp_query_pool, 0,
                        thread_pools->time_queries->time_queries.size * 2);
...
CommandBuffer* CommandBufferManager::get_secondary_command_buffer(u32 frame, u32 thread_index) {
  ...
  CommandBuffer* cb = &secondary_command_buffers[(pool_index * k_secondary_command_buffers_count) +
                                                 current_used_buffer];
  return cb;
}
