...
struct GpuDevice : public Service {
  // Helper methods
  static void fill_write_descriptor_sets(GpuDevice& gpu,
  ...
  void query_descriptor_set_layout(DescriptorSetLayoutHandle layout,
  ...
  void bind_texture_pages(PagePoolHandle pool_handle, TextureHandle handle, u32 x, u32 y, u32 width,
                          u32 height, u32 layer);

  void update_descriptor_set(DescriptorSetHandle set);

  // Misc //////////////////////////////////////////////////////////////
  void link_texture_sampler(TextureHandle texture,
                            SamplerHandle sampler);  // TODO: for now specify a sampler for a
                                                     // texture or use the default one.
  ...
  VkShaderModuleCreateInfo compile_shader(cstring code, u32 code_size, VkShaderStageFlagBits stage,
                                          cstring name);
  ...
  void queue_command_buffer(CommandBuffer* command_buffer);  // Queue command buffer that will not
                                                             // be executed until present is called.
  ...
  BufferHandle get_fullscreen_vertex_buffer()
      const;  // Returns a vertex buffer usable for fullscreen shaders that uses no vertices.
  RenderPassHandle get_swapchain_pass()
      const;  // Returns what is considered the final pass that writes to the swapchain.
  FramebufferHandle get_current_framebuffer()
      const;  // Returns the framebuffer for the active swapchain image
  ...
  VkDescriptorSet vulkan_bindless_descriptor_set_cached;  // Cached but will be removed with its
                                                          // associated DescriptorSet.
  DescriptorSetLayoutHandle bindless_descriptor_set_layout;
  DescriptorSetHandle bindless_descriptor_set;

  // Swapchain
  FramebufferHandle vulkan_swapchain_framebuffers[k_max_swapchain_images]{
      k_invalid_index, k_invalid_index, k_invalid_index};
  ...
  DescriptorSetLayoutHandle get_descriptor_set_layout(PipelineHandle pipeline_handle,
                                                      int layout_index);
  DescriptorSetLayoutHandle get_descriptor_set_layout(PipelineHandle pipeline_handle,
