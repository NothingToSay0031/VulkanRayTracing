...
struct GPUTimeQueriesManager {
  void init(GpuThreadFramePools* thread_frame_pools, Allocator* allocator, u16 queries_per_thread,
            u16 num_threads, u16 max_frames);
  void shutdown();

  void reset();
  u32 resolve(u32 current_frame,
              GPUTimeQuery* timestamps_to_fill);  // Returns the total queries for this frame.
  ...
  GpuPipelineStatistics
      frame_pipeline_statistics;  // Per frame statistics as sum of per-frame ones.

  u32 queries_per_thread = 0;
  u32 queries_per_frame = 0;
  u32 num_threads = 0;

  bool current_frame_resolved = false;  // Used to query the GPU only once per frame if
                                        // get_gpu_timestamps is called more than once per frame.
