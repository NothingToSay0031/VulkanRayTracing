    ...
    for (sizet oi = 0; oi < pass_outputs.size(); ++oi) {
      ...
      switch (output_creation.type) {
        ...
        case FrameGraphResourceType_Attachment: {
          ...
          output_creation.resource_info.texture.load_op =
          ...
          if (TextureFormat::has_depth(output_creation.resource_info.texture.format)) {
            output_creation.resource_info.texture.clear_values[0] =
                pass_output.value("clear_depth", 1.0f);
            output_creation.resource_info.texture.clear_values[1] =
    ...
    for (u32 n = 0; n < frame_graph->all_nodes.size; ++n) {
      ...
      for (u32 o = 0; o < parent_node->outputs.size; ++o) {
        ...
        rprint("Adding edge for resource %s from %s [%d] to %s [%d]\n", output_resource->name,
               parent_node->name, n, node->name, node_index)
#endif
...
static void create_render_pass(FrameGraph* frame_graph, FrameGraphNode* node) {
  ...
  for (u32 i = 0; i < node->outputs.size; ++i) {
    ...
    if (output_resource->type == FrameGraphResourceType_Attachment) {
      if (TextureFormat::has_depth(info.texture.format)) {
        render_pass_creation.set_depth_stencil_texture(
        ...
        render_pass_creation.add_attachment(
  ...
  for (u32 i = 0; i < node->inputs.size; ++i) {
    ...
    if (input_resource->type == FrameGraphResourceType_Attachment) {
      if (TextureFormat::has_depth(info.texture.format)) {
        ...
        render_pass_creation.add_attachment(info.texture.format,
                                            VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                                            RenderPassOperation::Load);
      }
    }
...
enum Enum { New = 0, Visited, Added, Count };  // enum Enum
};                                             // namespace FrameGraphNodeVisitStatus
...
void FrameGraph::compile() {
  ...
  for (u32 i = 0; i < all_nodes.size; ++i) {
    ...
    // NOTE(marco): we want to clear all edges first, then populate them. If we clear them inside
      ...
      if (!resource->resource_info.external &&
        ...
        if (resource->type == FrameGraphResourceType_Attachment) {
          ...
          TextureFlags::Mask texture_creation_flags =
              info.texture.compute ? (TextureFlags::Mask)(TextureFlags::RenderTarget_mask |
                                                          TextureFlags::Compute_mask)
                                   : TextureFlags::RenderTarget_mask;
          ...
          if (free_list.size > 0) {
            for (u32 r = 0; r < free_list.size; ++r) {
              ...
              texture_creation.set_data(nullptr)
                  .set_alias(alias_texture_handle)
                  ...
                  .set_flags(texture_creation_flags);
...
void FrameGraph::render(u32 current_frame_index, CommandBuffer* gpu_commands,
  ...
  for (u32 n = 0; n < nodes.size; ++n) {
    ...
    if (node->compute) {
      ...
      for (u32 i = 0; i < node->inputs.size; ++i) {
        ...
        if (input_resource->type == FrameGraphResourceType_Texture) {
          Texture* texture =
              gpu_commands->gpu_device->access_texture(resource->resource_info.texture.handle);
      ...
      for (u32 o = 0; o < node->outputs.size; ++o) {
        ...
        if (resource->type == FrameGraphResourceType_Attachment) {
          ...
          if (TextureFormat::has_depth(texture->vk_format)) {
            ...
            util_add_image_barrier(gpu_commands->gpu_device, gpu_commands->vk_command_buffer,
                                   texture, RESOURCE_STATE_UNORDERED_ACCESS, 0, 1, false);
          }
        }
      }
      ...
      for (u32 i = 0; i < node->inputs.size; ++i) {
        ...
        if (input_resource->type == FrameGraphResourceType_Texture) {
          ...
          util_add_image_barrier(gpu_commands->gpu_device, gpu_commands->vk_command_buffer, texture,
                                 RESOURCE_STATE_PIXEL_SHADER_RESOURCE, 0, 1,
                                 TextureFormat::has_depth(texture->vk_format));
        } else if (input_resource->type == FrameGraphResourceType_Attachment) {
          Texture* texture =
              gpu_commands->gpu_device->access_texture(resource->resource_info.texture.handle);
          ...
          if (!TextureFormat::has_depth_or_stencil(texture->vk_format)) {
            util_add_image_barrier(gpu_commands->gpu_device, gpu_commands->vk_command_buffer,
                                   texture, RESOURCE_STATE_RENDER_TARGET, 0, 1, false);
          } else {
            util_add_image_barrier(gpu_commands->gpu_device, gpu_commands->vk_command_buffer,
                                   texture, RESOURCE_STATE_DEPTH_WRITE, 0, 1, true);
          }
        }
      }
...
void FrameGraph::reload_shaders(RenderScene& scene, Allocator* resident_allocator,
  ...
  if (ImGui::CollapsingHeader("Nodes")) {
    for (u32 n = 0; n < nodes.size; ++n) {
      ...
      for (u32 i = 0; i < node->inputs.size; ++i) {
        ...
        ImGui::Text("\t\t%s %u %u", resource->name, resource->resource_info.texture.handle.index,
                    resource->resource_info.buffer.handle.index);
      }
...
void FrameGraph::add_resource(cstring name, FrameGraphResourceType type,
...
void FrameGraphResourceCache::shutdown() {
  ...
  while (it.is_valid()) {
    ...
    if ((resource->type == FrameGraphResourceType_Texture ||
         resource->type == FrameGraphResourceType_Attachment) &&
...
FrameGraphResourceHandle FrameGraphBuilder::create_node_output(
  ...
  if (creation.type != FrameGraphResourceType_Reference) {
    ...
    if (producer_node->enabled) {
      ...
      resource_cache.resource_map.insert(hash_bytes((void*)resource->name, strlen(creation.name)),
...
FrameGraphResourceHandle FrameGraphBuilder::create_node_input(
...
FrameGraphNode* FrameGraphBuilder::get_node(cstring name) {
  ...
  FrameGraphNode* node =
      (FrameGraphNode*)node_cache.nodes.access_resource(node_cache.node_map.get(it));
...
void FrameGraphBuilder::add_resource(cstring name, FrameGraphResourceType type,
                                     FrameGraphResourceInfo resource_info) {
...
FrameGraphResourceInfo& FrameGraphResourceInfo::set_buffer(sizet size, VkBufferUsageFlags flags,
                                                           BufferHandle handle) {
  buffer.size = size;
  buffer.flags = flags;
...
FrameGraphResourceInfo& FrameGraphResourceInfo::set_external_texture_2d(u32 width, u32 height,
                                                                        VkFormat format,
...
FrameGraphResourceInfo& FrameGraphResourceInfo::set_external_texture_3d(u32 width, u32 height,
                                                                        u32 depth, VkFormat format,
                                                                        VkImageUsageFlags flags,
                                                                        TextureHandle handle) {
  texture.width = width;
  texture.height = height;
