...
struct Mesh {
  ...
  bool is_transparent() const {
    return (pbr_material.flags & (DrawFlags_AlphaMask | DrawFlags_Transparent)) != 0;
  }
  bool is_double_sided() const {
    return (pbr_material.flags & DrawFlags_DoubleSided) == DrawFlags_DoubleSided;
  }
...
struct DepthPrePass : public FrameGraphRenderPass {
  void render(u32 current_frame_index, CommandBuffer* gpu_commands,
              RenderScene* render_scene) override;

  void prepare_draws(RenderScene& scene, FrameGraph* frame_graph, Allocator* resident_allocator,
...
struct DepthPyramidPass : public FrameGraphRenderPass {
  ...
  void post_render(u32 current_frame_index, CommandBuffer* gpu_commands, FrameGraph* frame_graph,
                   RenderScene* render_scene) override;
...
struct GBufferPass : public FrameGraphRenderPass {
  void pre_render(u32 current_frame_index, CommandBuffer* gpu_commands, FrameGraph* frame_graph,
                  RenderScene* render_scene) override;
  void render(u32 current_frame_index, CommandBuffer* gpu_commands,
              RenderScene* render_scene) override;
...
struct LightPass : public FrameGraphRenderPass {
  ...
  void free_gpu_resources(GpuDevice& gpu) override;
  void update_dependent_resources(GpuDevice& gpu, FrameGraph* frame_graph,
                                  RenderScene* render_scene) override;
...
struct IndirectPass : public FrameGraphRenderPass {
  ...
  i32 irradiance_probe_size =
      6;  // Irradiance is a 6x6 quad with 1 pixel borders for bilinear filtering, total 8x8
...
struct ReflectionsPass : public FrameGraphRenderPass {
  ...
  void reload_shaders(RenderScene& scene, FrameGraph* frame_graph, Allocator* resident_allocator,
                      StackAllocator* scratch_allocator) override;
  void update_dependent_resources(GpuDevice& gpu, FrameGraph* frame_graph,
                                  RenderScene* render_scene) override;
...
struct RenderScene {
  ...
  virtual void add_mesh(cstring filename, cstring path, StackAllocator* temp_allocator,
  ...
  CommandBuffer* update_physics(f32 delta_time, f32 air_density, f32 spring_stiffness,
                                f32 spring_damping, vec3s wind_direction, bool reset_simulation);
  ...
  void add_scene_descriptors(DescriptorSetCreation& descriptor_set_creation,
                             GpuTechniquePass& pass);
  void add_mesh_descriptors(DescriptorSetCreation& descriptor_set_creation, GpuTechniquePass& pass);
  void add_meshlet_descriptors(DescriptorSetCreation& descriptor_set_creation,
                               GpuTechniquePass& pass);
  void add_debug_descriptors(DescriptorSetCreation& descriptor_set_creation,
                             GpuTechniquePass& pass);
  void add_lighting_descriptors(DescriptorSetCreation& descriptor_set_creation,
  ...
  u32 volumetric_fog_box_color = hydra::Color::yellow;
...
struct FrameRenderer {
  void init(Allocator* resident_allocator, Renderer* renderer, FrameGraph* frame_graph,
            SceneGraph* scene_graph, RenderScene* scene);
  void shutdown();
...
struct DrawTask : public enki::ITaskSet {
  ...
  void init(GpuDevice* gpu_, FrameGraph* frame_graph_, Renderer* renderer_, ImGuiService* imgui_,
            GpuVisualProfiler* gpu_profiler_, RenderScene* scene_, FrameRenderer* frame_renderer);
