...
void glTFScene::get_mesh_vertex_buffer(glTF::glTF& gltf_scene, u32 buffers_offset,
                                       i32 accessor_index, u32 flag,
                                       BufferHandle& out_buffer_handle, u32& out_buffer_offset,
                                       u32& out_flags) {
  if (accessor_index != -1) {
...
void glTFScene::fill_pbr_material(glTF::glTF& gltf_scene, Renderer& renderer,
                                  glTF::Material& material, PBRMaterial& pbr_material) {
  ...
  // Alpha cutoff
  pbr_material.alpha_cutoff =
      material.alpha_cutoff != glTF::INVALID_FLOAT_VALUE ? material.alpha_cutoff : 1.f;
  ...
  if (material.pbr_metallic_roughness != nullptr) {
    if (material.pbr_metallic_roughness->base_color_factor_count != 0) {
      ...
      memcpy(pbr_material.base_color_factor.raw, material.pbr_metallic_roughness->base_color_factor,
             sizeof(vec4s));
    } else {
    ...
    pbr_material.roughness =
        material.pbr_metallic_roughness->roughness_factor != glTF::INVALID_FLOAT_VALUE
            ? material.pbr_metallic_roughness->roughness_factor
            : 1.f;
    pbr_material.metallic =
        material.pbr_metallic_roughness->metallic_factor != glTF::INVALID_FLOAT_VALUE
            ? material.pbr_metallic_roughness->metallic_factor
            : 0.f;

    pbr_material.diffuse_texture_index =
        get_material_texture(gpu, gltf_scene, material.pbr_metallic_roughness->base_color_texture);
    pbr_material.roughness_texture_index = get_material_texture(
        gpu, gltf_scene, material.pbr_metallic_roughness->metallic_roughness_texture);
  }

  if (material.emissive_texture != nullptr) {
    pbr_material.emissive_texture_index =
        get_material_texture(gpu, gltf_scene, material.emissive_texture);
  }
  ...
  pbr_material.occlusion_texture_index = get_material_texture(
      gpu, gltf_scene,
      (material.occlusion_texture != nullptr) ? material.occlusion_texture->index : -1);
  pbr_material.normal_texture_index = get_material_texture(
      gpu, gltf_scene, (material.normal_texture != nullptr) ? material.normal_texture->index : -1);
...
u16 glTFScene::get_material_texture(GpuDevice& gpu, glTF::glTF& gltf_scene,
...
void glTFScene::init(SceneGraph* scene_graph_, Allocator* resident_allocator_,
                     Renderer* renderer_) {
...
void glTFScene::add_mesh(cstring filename, cstring path, StackAllocator* temp_allocator,
  ...
  for (u32 buffer_index = 0; buffer_index < gltf_scene.buffers_count; ++buffer_index) {
    ...
    BufferResource* br = renderer->create_buffer(flags, ResourceUsageType::Immutable,
                                                 buffer.byte_length, buffer_data, buffer_name);
    buffers.push(*br);
  }
  ...
  for (u32 mi = 0; mi < gltf_scene.meshes_count; ++mi) {
    ...
    for (u32 p = 0; p < mesh.primitives_count; ++p) {
      ...
      /*if ( mesh_primitive.material != glTF::INVALID_INT_VALUE ) {
          ...
          if ( ( material.alpha_mode.data != nullptr && strcmp( material.alpha_mode.data, "MASK" )
      == 0 ) || ( material.alpha_mode.data != nullptr && strcmp( material.alpha_mode.data, "BLEND" )
      == 0 ) ) { continue;
          }
      }*/
      ...
      const i32 position_accessor_index = gltf_get_attribute_accessor_index(
          mesh_primitive.attributes, mesh_primitive.attribute_count, "POSITION");
      glTF::Accessor& position_buffer_accessor = gltf_scene.accessors[position_accessor_index];
      glTF::BufferView& position_buffer_view =
          gltf_scene.buffer_views[position_buffer_accessor.buffer_view];
      i32 position_data_offset = glTF::get_data_offset(position_buffer_accessor.byte_offset,
      ...
      vec3s position_min{position_buffer_accessor.min[0], position_buffer_accessor.min[1],
                         position_buffer_accessor.min[2]};
      vec3s position_max{position_buffer_accessor.max[0], position_buffer_accessor.max[1],
                         position_buffer_accessor.max[2]};
      vec3s bounding_center = glms_vec3_add(position_min, position_max);
      bounding_center = glms_vec3_divs(bounding_center, 2.0f);

      // Calculate bounding sphere radius
      f32 radius = hydra::max(glms_vec3_distance(position_max, bounding_center),
                               glms_vec3_distance(position_min, bounding_center));
      ...
      if (normal_accessor_index != -1) {
        ...
        i32 normal_data_offset = glTF::get_data_offset(normal_buffer_accessor.byte_offset,
                                                       normal_buffer_view.byte_offset);
        normals = (f32*)((u8*)buffers_data[normal_buffer_view.buffer] + normal_data_offset);
      }

      // Vertex texture coords
      const i32 tex_coord_accessor_index = gltf_get_attribute_accessor_index(
          mesh_primitive.attributes, mesh_primitive.attribute_count, "TEXCOORD_0");
      ...
      if (tex_coord_accessor_index != -1) {
        ...
        i32 tex_coord_data_offset = glTF::get_data_offset(tex_coord_buffer_accessor.byte_offset,
                                                          tex_coord_buffer_view.byte_offset);
        tex_coords =
            (f32*)((u8*)buffers_data[tex_coord_buffer_view.buffer] + tex_coord_data_offset);
      }
      ...
      if (tangent_accessor_index != -1) {
        ...
        i32 tangent_data_offset = glTF::get_data_offset(tangent_buffer_accessor.byte_offset,
      ...
      get_mesh_vertex_buffer(gltf_scene, buffers_offset, position_accessor_index, 0,
                             mesh.position_buffer, mesh.position_offset, mesh.pbr_material.flags);
      get_mesh_vertex_buffer(gltf_scene, buffers_offset, tangent_accessor_index,
                             DrawFlags_HasTangents, mesh.tangent_buffer, mesh.tangent_offset,
                             mesh.pbr_material.flags);
      get_mesh_vertex_buffer(gltf_scene, buffers_offset, normal_accessor_index,
                             DrawFlags_HasNormals, mesh.normal_buffer, mesh.normal_offset,
                             mesh.pbr_material.flags);
      get_mesh_vertex_buffer(gltf_scene, buffers_offset, tex_coord_accessor_index,
                             DrawFlags_HasTexCoords, mesh.texcoord_buffer, mesh.texcoord_offset,
                             mesh.pbr_material.flags);

      const i32 joints_accessor_index = gltf_get_attribute_accessor_index(
          mesh_primitive.attributes, mesh_primitive.attribute_count, "JOINTS_0");
      const i32 weights_accessor_index = gltf_get_attribute_accessor_index(
          mesh_primitive.attributes, mesh_primitive.attribute_count, "WEIGHTS_0");

      get_mesh_vertex_buffer(gltf_scene, buffers_offset, joints_accessor_index, DrawFlags_HasJoints,
                             mesh.joints_buffer, mesh.joints_offset, mesh.pbr_material.flags);
      get_mesh_vertex_buffer(gltf_scene, buffers_offset, weights_accessor_index,
                             DrawFlags_HasWeights, mesh.weights_buffer, mesh.weights_offset,
                             mesh.pbr_material.flags);
      ...
      i32 index_data_offset =
          glTF::get_data_offset(indices_accessor.byte_offset, indices_buffer_view.byte_offset);
      ...
      const sizet max_meshlets =
          meshopt_buildMeshletsBound(indices_accessor.count, max_vertices, max_triangles);
      ...
      meshlet_vertex_indices.init(temp_allocator, max_meshlets * max_vertices,
                                  max_meshlets * max_vertices);

      Array<u8> meshlet_triangles;
      meshlet_triangles.init(temp_allocator, max_meshlets * max_triangles * 3,
                             max_meshlets * max_triangles * 3);

      sizet meshlet_count = meshopt_buildMeshlets(
          local_meshlets.data, meshlet_vertex_indices.data, meshlet_triangles.data, indices,
      ...
      for (u32 m = 0; m < meshlet_count; ++m) {
        ...
        meshlet.center =
            vec3s{meshlet_bounds.center[0], meshlet_bounds.center[1], meshlet_bounds.center[2]};
        ...
        meshlets_data.set_capacity(meshlets_data.size + local_meshlet.vertex_count +
                                   index_group_count);

        for (u32 i = 0; i < meshlet.vertex_count; ++i) {
          const u32 vertex_index =
              meshlet_vertex_offset + meshlet_vertex_indices[local_meshlet.vertex_offset + i];
        ...
        const u32* index_groups =
            reinterpret_cast<const u32*>(meshlet_triangles.data + local_meshlet.triangle_offset);
  ...
  material_creation.set_name("material_no_cull_opaque")
      .set_technique(main_technique)
      .set_render_index(0);
  ...
  BufferCreation bc{};
  bc.set(VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR |
             VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT_KHR,
         ResourceUsageType::Immutable, geometry_transform_buffer_size)
      .set_persistent(true)
      .set_name("geometry_transform_buffer");
  ...
  for (u32 mesh_index = 0; mesh_index < transform_count; ++mesh_index) {
    ...
    geometry.geometry.triangles.vertexData.deviceAddress =
        renderer->gpu->get_buffer_device_address(mesh.position_buffer) + mesh.position_offset;
    ...
    geometry.geometry.triangles.indexData.deviceAddress =
        renderer->gpu->get_buffer_device_address(mesh.index_buffer) + mesh.index_offset;
    geometry.geometry.triangles.transformData.deviceAddress =
        renderer->gpu->get_buffer_device_address(geometry_transform_buffer);
  ...
  memcpy(gpu_geometry_transform_buffer->mapped_data, geometry_transform.data,
         geometry_transform_buffer_size);
  ...
  for (u32 animation_index = 0; animation_index < gltf_scene.animations_count; ++animation_index) {
    ...
    animation.channels.init(resident_allocator, gltf_animation.channels_count,
                            gltf_animation.channels_count);
    for (u32 channel_index = 0; channel_index < gltf_animation.channels_count; ++channel_index) {
      ...
      channel.target_type = (hydra::AnimationChannel::TargetType)gltf_channel.target_type;
    }

    animation.samplers.init(resident_allocator, gltf_animation.samplers_count,
                            gltf_animation.samplers_count);
    for (u32 sampler_index = 0; sampler_index < gltf_animation.samplers_count; ++sampler_index) {
      ...
      sampler.interpolation_type =
          (hydra::AnimationSampler::Interpolation)gltf_sampler.interpolation;
      ...
      // Copy keyframe data
      {
        glTF::Accessor& buffer_accessor =
            gltf_scene.accessors[gltf_sampler.input_keyframe_buffer_index];
        glTF::BufferView& buffer_view = gltf_scene.buffer_views[buffer_accessor.buffer_view];

        i32 byte_offset =
            glTF::get_data_offset(buffer_accessor.byte_offset, buffer_view.byte_offset);
      ...
      {
        glTF::Accessor& buffer_accessor =
            gltf_scene.accessors[gltf_sampler.output_keyframe_buffer_index];
        glTF::BufferView& buffer_view = gltf_scene.buffer_views[buffer_accessor.buffer_view];
        ...
        sampler.data =
            (vec4s*)rallocaa(sizeof(vec4s) * buffer_accessor.count, resident_allocator, 16);
        ...
        switch (buffer_accessor.type) {
          ...
          case glTF::Accessor::Vec4: {
            ...
            for (u32 i = 0; i < buffer_accessor.count; ++i) {
              sampler.data[i] = vec4s{animation_data[i * 4], animation_data[i * 4 + 1],
                                      animation_data[i * 4 + 2], animation_data[i * 4 + 3]};
            }
            break;
          }
          default: {
  ...
  for (u32 si = 0; si < gltf_scene.skins_count; ++si) {
    ...
    glTF::Accessor& buffer_accessor =
        gltf_scene.accessors[gltf_skin.inverse_bind_matrices_buffer_index];
    glTF::BufferView& buffer_view = gltf_scene.buffer_views[buffer_accessor.buffer_view];
    ...
    skin.inverse_bind_matrices =
        (mat4s*)rallocaa(sizeof(mat4s) * buffer_accessor.count, resident_allocator, 16);
    ...
    bc.reset()
        .set(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, ResourceUsageType::Dynamic,
             sizeof(mat4s) * gltf_scene.nodes_count)
  ...
  rprint(
      ...
      "Buffers %f seconds\n",
      filename, time_delta_seconds(start_scene_loading, end_loading),
...
void glTFScene::prepare_draws(Renderer* renderer, StackAllocator* scratch_allocator,
  ...
  buffer_creation.reset()
      .set(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, ResourceUsageType::Dynamic, sizeof(GpuSceneData))
      .set_name("scene_cb");
  scene_cb = renderer->gpu->create_buffer(buffer_creation);

  buffer_creation.reset()
      .set(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, ResourceUsageType::Immutable,
           sizeof(u32) * meshlets_data.size)
      .set_name("meshlet_data_sb")
      .set_data(meshlets_data.data);
  ...
  buffer_creation.reset()
      .set(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, ResourceUsageType::Dynamic,
           sizeof(GpuMaterialData) * meshes.size)
      .set_name("meshes_sb");
  ...
  buffer_creation.reset()
      .set(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, ResourceUsageType::Dynamic,
           sizeof(vec4s) * meshes.size)
      .set_name("mesh_bound_sb");
  mesh_bounds_sb = renderer->gpu->create_buffer(buffer_creation);

  // Create mesh instances ssbo
  buffer_creation.reset()
      .set(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, ResourceUsageType::Dynamic,
           sizeof(GpuMeshInstanceData) * mesh_instances.size)
      .set_name("mesh_instances_sb");
  ...
  for (u32 i = 0; i < k_max_frames; ++i) {
    ...
    buffer_creation.reset()
        .set(VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
             ResourceUsageType::Dynamic, mesh_instances.size * sizeof(GpuMeshDrawCommand) * 2)
        .set_name("early_draw_commands_sb");
    mesh_task_indirect_early_commands_sb[i] = renderer->gpu->create_buffer(buffer_creation);

    buffer_creation.reset()
        .set(VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
             ResourceUsageType::Dynamic, mesh_instances.size * sizeof(GpuMeshDrawCommand) * 2)
        .set_name("culled_draw_commands_sb");
    mesh_task_indirect_culled_commands_sb[i] = renderer->gpu->create_buffer(buffer_creation);

    buffer_creation.reset()
        .set(VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
             ResourceUsageType::Dynamic, mesh_instances.size * sizeof(GpuMeshDrawCommand) * 2)
        .set_name("late_draw_commands_sb");
  ...
  // Create debug draw buffers
  {
    ...
    buffer_creation.reset()
        .set(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, ResourceUsageType::Dynamic,
             k_max_lines * sizeof(vec4s) * 2)
        .set_name("debug_line_sb");
    debug_line_sb = renderer->gpu->create_buffer(buffer_creation);

    buffer_creation.reset()
        .set(VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
             ResourceUsageType::Dynamic, sizeof(vec4s))
        .set_name("debug_line_count_sb");
  ...
  for (u32 m = 0; m < meshes.size; ++m) {
    ...
    if (mesh.has_skinning()) {
      pass_index =
          main_technique->name_hash_to_index.get(hash_calculate("transparent_skinning_no_cull"));
      depth_pass_index =
          main_technique->name_hash_to_index.get(hash_calculate("depth_pre_skinning"));
    } else {
    ...
    DescriptorSetLayoutHandle layout = renderer->gpu->get_descriptor_set_layout(
        main_technique->passes[pass_index].pipeline, k_material_descriptor_set_index);
    // DescriptorSetLayout* ds = renderer->gpu->access_descriptor_set_layout( layout );
    ds_creation.buffer(scene_cb, 0)
        .buffer(meshes_sb, 2)
        .buffer(mesh_instances_sb, 10)
        .buffer(mesh_bounds_sb, 12)
        .buffer(debug_line_sb, 20)
        .buffer(debug_line_count_sb, 21)
        .buffer(debug_line_commands_sb, 22)
        .buffer(mesh_bounds_sb, 25)
        .set_layout(layout);
    ...
    layout = renderer->gpu->get_descriptor_set_layout(
        main_technique->passes[depth_pass_index].pipeline, k_material_descriptor_set_index);
  ...
  // Create meshlets index buffer, that will be used to emulate meshlets if mesh shaders are not
  // present.
  {
    BufferCreation bc{};
    bc.set(VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
           ResourceUsageType::Stream, meshlets_index_count * sizeof(u32) * 8)
        .set_name("meshlets_index_buffer");
    ...
    bc.reset()
        .set(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, ResourceUsageType::Stream,
             (meshlets.size * 2 /* not sure about max number here */) * sizeof(u32) * 2)
        .set_name("meshlets_instances_buffer");
  ...
  // Meshlet and meshlet emulation descriptors
  {
    ...
    if (renderer->gpu->mesh_shaders_extension_present) {
      ...
      DescriptorSetLayoutHandle layout =
          meshlet_index != u16_max ? renderer->gpu->get_descriptor_set_layout(
                                         meshlet_pass.pipeline, k_material_descriptor_set_index)
                                   : k_invalid_layout;
      ...
      for (u32 i = 0; i < k_max_frames; ++i) {
        ...
        ds_creation.buffer(mesh_task_indirect_early_commands_sb[i], 6)
        ...
        ds_creation.buffer(mesh_task_indirect_late_commands_sb[i], 6)
            .buffer(mesh_task_indirect_count_late_sb[i], 7)
            .set_layout(layout);
    ...
    DescriptorSetLayoutHandle meshlet_emulation_layout = renderer->gpu->get_descriptor_set_layout(
        meshlet_emulation_pass.pipeline, k_material_descriptor_set_index);
    ...
    for (u32 i = 0; i < k_max_frames; ++i) {
      ...
      ds_creation.buffer(mesh_task_indirect_early_commands_sb[i], 6)
          .buffer(mesh_task_indirect_count_early_sb[i], 7)
          .buffer(meshlets_instances_sb[i],
                  meshlet_emulation_pass.get_binding_index("MeshletInstances"))
          .set_layout(meshlet_emulation_layout);
...

  {
    buffer_creation.reset()
        .set(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, ResourceUsageType::Dynamic,
             sizeof(GpuLight) * k_num_lights)
        .set_name("light_array");
