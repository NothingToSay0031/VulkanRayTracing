...
#define check(result) \
  RASSERTM(result == VK_SUCCESS, "Vulkan assert code %u, '%s'", result, string_VkResult(result))
...
#define HYDRA_GPU_DEVICE_RESOURCE_TRACKING

#if defined(HYDRA_GPU_DEVICE_RESOURCE_TRACKING)
//
//
struct ResourceTracker {
  void init(Allocator* allocator) { resources_to_names.init(allocator, 64); }
  ...
  void track_create_resource(ResourceUpdateType::Enum type, u32 index, cstring name) {
    ...
    if (track_resource && tracked_resource_type == type &&
        ((tracked_resource_index == index) || track_all_indices_per_type)) {
  ...
  hydra::FlatHashMap<u32, u32> resources_to_names;
...
#endif  // HYDRA_GPU_DEVICE_RESOURCE_TRACKING
...
static VkBool32 debug_utils_callback(VkDebugUtilsMessageSeverityFlagBitsEXT severity,
                                     ...
                                     void* user_data) {
  bool triggerBreak = severity & (VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT |
...
bool GpuDevice::get_family_queue(VkPhysicalDevice physical_device) {
  ...
  VkQueueFamilyProperties* queue_families = (VkQueueFamilyProperties*)ralloca(
  ...
  for (; family_index < queue_family_count; ++family_index) {
    ...
    if (queue_family.queueCount > 0 &&
      ...
      vkGetPhysicalDeviceSurfaceSupportKHR(physical_device, family_index, vulkan_window_surface,
                                           &surface_supported);
...
void GpuDevice::init(const GpuDeviceCreation& creation) {
  ...
  const VkValidationFeatureEnableEXT featuresRequested[] = {
      VK_VALIDATION_FEATURE_ENABLE_GPU_ASSISTED_EXT, VK_VALIDATION_FEATURE_ENABLE_SYNCHRONIZATION_VALIDATION_EXT /*, VK_VALIDATION_FEATURE_ENABLE_BEST_PRACTICES_EXT*/};
...
#ifdef VULKAN_DEBUG_REPORT
  {
    ...
    VkExtensionProperties* extensions = (VkExtensionProperties*)ralloca(
        sizeof(VkExtensionProperties) * num_instance_extensions, temp_allocator);
    ...
    if (!debug_utils_extension_present) {
      ...
      vkCreateDebugUtilsMessengerEXT(vulkan_instance, &debug_messenger_create_info,
  ...
  VkPhysicalDevice* gpus =
      (VkPhysicalDevice*)ralloca(sizeof(VkPhysicalDevice) * num_physical_device, temp_allocator);
...

  {
    ...
    vkEnumerateDeviceExtensionProperties(vulkan_physical_device, nullptr, &device_extension_count,
                                         nullptr);
    VkExtensionProperties* extensions = (VkExtensionProperties*)ralloca(
        sizeof(VkExtensionProperties) * device_extension_count, temp_allocator);
    vkEnumerateDeviceExtensionProperties(vulkan_physical_device, nullptr, &device_extension_count,
                                         extensions);
    for (size_t i = 0; i < device_extension_count; i++) {
      if (!creation.force_disable_dynamic_rendering &&
          !strcmp(extensions[i].extensionName, VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME)) {
  ...
  if (fragment_shading_rate_present) {
    min_fragment_shading_rate_texel_size =
        fragment_shading_rate_properties.minFragmentShadingRateAttachmentTexelSize;
  }
  ...
  ray_tracing_pipeline_features = VkPhysicalDeviceRayTracingPipelineFeaturesKHR{
  ...
  // For the feature to be correctly working, we need both the possibility to partially bind a
  // descriptor, as some entries in the bindless array will be empty, and SpirV runtime descriptors.
  bindless_supported =
  ...
  VkQueueFamilyProperties* queue_families = (VkQueueFamilyProperties*)ralloca(
      sizeof(VkQueueFamilyProperties) * queue_family_count, temp_allocator);
  vkGetPhysicalDeviceQueueFamilyProperties(vulkan_physical_device, &queue_family_count,
                                           queue_families);

  u32 main_queue_family_index = u32_max, transfer_queue_family_index = u32_max,
      compute_queue_family_index = u32_max, present_queue_family_index = u32_max;
  ...
  for (u32 fi = 0; fi < queue_family_count; ++fi) {
    ...
    if ((queue_family.queueFlags & (VK_QUEUE_GRAPHICS_BIT | VK_QUEUE_COMPUTE_BIT)) ==
      ...
      RASSERT((queue_family.queueFlags & VK_QUEUE_SPARSE_BINDING_BIT) ==
  ...
  VkPhysicalDeviceMeshShaderFeaturesNV mesh_shaders_features = {
  ...
  result = vkCreateDevice(vulkan_physical_device, &device_create_info, vulkan_allocation_callbacks,
                          &vulkan_device);
  check(result);
  ...
  if (debug_utils_extension_present) {
    pfnSetDebugUtilsObjectNameEXT = (PFN_vkSetDebugUtilsObjectNameEXT)vkGetDeviceProcAddr(
        vulkan_device, "vkSetDebugUtilsObjectNameEXT");
    pfnCmdBeginDebugUtilsLabelEXT = (PFN_vkCmdBeginDebugUtilsLabelEXT)vkGetDeviceProcAddr(
        vulkan_device, "vkCmdBeginDebugUtilsLabelEXT");
    pfnCmdEndDebugUtilsLabelEXT = (PFN_vkCmdEndDebugUtilsLabelEXT)vkGetDeviceProcAddr(
        vulkan_device, "vkCmdEndDebugUtilsLabelEXT");
  }

  if (dynamic_rendering_extension_present) {
    vkCmdBeginRenderingKHR =
        (PFN_vkCmdBeginRenderingKHR)vkGetDeviceProcAddr(vulkan_device, "vkCmdBeginRenderingKHR");
    vkCmdEndRenderingKHR =
        (PFN_vkCmdEndRenderingKHR)vkGetDeviceProcAddr(vulkan_device, "vkCmdEndRenderingKHR");
  }

  if (synchronization2_extension_present) {
    vkQueueSubmit2KHR =
        (PFN_vkQueueSubmit2KHR)vkGetDeviceProcAddr(vulkan_device, "vkQueueSubmit2KHR");
    vkCmdPipelineBarrier2KHR = (PFN_vkCmdPipelineBarrier2KHR)vkGetDeviceProcAddr(
        vulkan_device, "vkCmdPipelineBarrier2KHR");
  }

  if (mesh_shaders_extension_present) {
    vkCmdDrawMeshTasksNV =
        (PFN_vkCmdDrawMeshTasksNV)vkGetDeviceProcAddr(vulkan_device, "vkCmdDrawMeshTasksNV");
    vkCmdDrawMeshTasksIndirectNV = (PFN_vkCmdDrawMeshTasksIndirectNV)vkGetDeviceProcAddr(
        vulkan_device, "vkCmdDrawMeshTasksIndirectNV");
    vkCmdDrawMeshTasksIndirectCountNV = (PFN_vkCmdDrawMeshTasksIndirectCountNV)vkGetDeviceProcAddr(
        vulkan_device, "vkCmdDrawMeshTasksIndirectCountNV");
  }

  if (fragment_shading_rate_present) {
    vkGetPhysicalDeviceFragmentShadingRatesKHR =
        (PFN_vkGetPhysicalDeviceFragmentShadingRatesKHR)vkGetInstanceProcAddr(
            vulkan_instance, "vkGetPhysicalDeviceFragmentShadingRatesKHR");
    vkCmdSetFragmentShadingRateKHR = (PFN_vkCmdSetFragmentShadingRateKHR)vkGetDeviceProcAddr(
        vulkan_device, "vkCmdSetFragmentShadingRateKHR");

    u32 shading_rates_count = 0;
    vkGetPhysicalDeviceFragmentShadingRatesKHR(vulkan_physical_device, &shading_rates_count,
                                               nullptr);

    fragment_shading_rates.init(allocator, shading_rates_count, shading_rates_count);
    memset(fragment_shading_rates.data, 0,
           sizeof(VkPhysicalDeviceFragmentShadingRateKHR) * shading_rates_count);

    for (u32 fsr_index = 0; fsr_index < shading_rates_count; ++fsr_index) {
      fragment_shading_rates[fsr_index].sType =
          VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FRAGMENT_SHADING_RATE_KHR;
    }

    vkGetPhysicalDeviceFragmentShadingRatesKHR(vulkan_physical_device, &shading_rates_count,
                                               fragment_shading_rates.data);
  }

  if (ray_tracing_present) {
    vkCreateRayTracingPipelinesKHR = (PFN_vkCreateRayTracingPipelinesKHR)vkGetDeviceProcAddr(
        vulkan_device, "vkCreateRayTracingPipelinesKHR");
    vkCmdTraceRaysKHR =
        (PFN_vkCmdTraceRaysKHR)vkGetDeviceProcAddr(vulkan_device, "vkCmdTraceRaysKHR");
    vkCmdTraceRaysIndirectKHR = (PFN_vkCmdTraceRaysIndirectKHR)vkGetDeviceProcAddr(
        vulkan_device, "vkCmdTraceRaysIndirectKHR");

    vkGetRayTracingShaderGroupHandlesKHR =
        (PFN_vkGetRayTracingShaderGroupHandlesKHR)vkGetDeviceProcAddr(
            vulkan_device, "vkGetRayTracingShaderGroupHandlesKHR");
    vkGetBufferDeviceAddressKHR = (PFN_vkGetBufferDeviceAddressKHR)vkGetDeviceProcAddr(
        vulkan_device, "vkGetBufferDeviceAddressKHR");
    vkGetAccelerationStructureDeviceAddressKHR =
        (PFN_vkGetAccelerationStructureDeviceAddressKHR)vkGetDeviceProcAddr(
            vulkan_device, "vkGetAccelerationStructureDeviceAddressKHR");
    vkGetAccelerationStructureBuildSizesKHR =
        (PFN_vkGetAccelerationStructureBuildSizesKHR)vkGetDeviceProcAddr(
            vulkan_device, "vkGetAccelerationStructureBuildSizesKHR");

    vkCreateAccelerationStructureKHR = (PFN_vkCreateAccelerationStructureKHR)vkGetDeviceProcAddr(
        vulkan_device, "vkCreateAccelerationStructureKHR");
    vkDestroyAccelerationStructureKHR = (PFN_vkDestroyAccelerationStructureKHR)vkGetDeviceProcAddr(
        vulkan_device, "vkDestroyAccelerationStructureKHR");
    vkCmdBuildAccelerationStructuresKHR =
        (PFN_vkCmdBuildAccelerationStructuresKHR)vkGetDeviceProcAddr(
            vulkan_device, "vkCmdBuildAccelerationStructuresKHR");
    vkCmdBuildAccelerationStructuresIndirectKHR =
        (PFN_vkCmdBuildAccelerationStructuresIndirectKHR)vkGetDeviceProcAddr(
            vulkan_device, "vkCmdBuildAccelerationStructuresIndirectKHR");
    vkCmdWriteAccelerationStructuresPropertiesKHR =
        (PFN_vkCmdWriteAccelerationStructuresPropertiesKHR)vkGetDeviceProcAddr(
            vulkan_device, "vkCmdWriteAccelerationStructuresPropertiesKHR");
    vkCmdCopyAccelerationStructureKHR = (PFN_vkCmdCopyAccelerationStructureKHR)vkGetDeviceProcAddr(
        vulkan_device, "vkCmdCopyAccelerationStructureKHR");
    vkCmdCopyMemoryToAccelerationStructureKHR =
        (PFN_vkCmdCopyMemoryToAccelerationStructureKHR)vkGetDeviceProcAddr(
            vulkan_device, "vkCmdCopyMemoryToAccelerationStructureKHR");

    vkBuildAccelerationStructuresKHR = (PFN_vkBuildAccelerationStructuresKHR)vkGetDeviceProcAddr(
        vulkan_device, "vkBuildAccelerationStructuresKHR");
    vkWriteAccelerationStructuresPropertiesKHR =
        (PFN_vkWriteAccelerationStructuresPropertiesKHR)vkGetDeviceProcAddr(
            vulkan_device, "vkWriteAccelerationStructuresPropertiesKHR");
    vkCopyAccelerationStructureKHR = (PFN_vkCopyAccelerationStructureKHR)vkGetDeviceProcAddr(
        vulkan_device, "vkCopyAccelerationStructureKHR");
    vkCopyMemoryToAccelerationStructureKHR =
        (PFN_vkCopyMemoryToAccelerationStructureKHR)vkGetDeviceProcAddr(
            vulkan_device, "vkCopyMemoryToAccelerationStructureKHR");
  }
  ...
  vkGetDeviceQueue(vulkan_device, compute_queue_family_index, compute_queue_index,
  ...
  // const TextureFormat::Enum swapchain_formats[] = { TextureFormat::B8G8R8A8_UNORM,
  // TextureFormat::R8G8B8A8_UNORM, TextureFormat::B8G8R8X8_UNORM, TextureFormat::B8G8R8X8_UNORM };
  ...
  u32 supported_count;
  vkGetPhysicalDeviceSurfaceFormatsKHR(vulkan_physical_device, vulkan_window_surface,
                                       &supported_count, NULL);
  VkSurfaceFormatKHR* supported_formats =
      (VkSurfaceFormatKHR*)ralloca(sizeof(VkSurfaceFormatKHR) * supported_count, temp_allocator);
  vkGetPhysicalDeviceSurfaceFormatsKHR(vulkan_physical_device, vulkan_window_surface,
                                       &supported_count, supported_formats);
  ...
  swapchain_output.set_depth_stencil_operations(RenderPassOperation::Clear,
  ...
  swapchain_output.color(vulkan_surface_format.format, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
                         RenderPassOperation::Clear);
  ...
  result = vkCreateDescriptorPool(vulkan_device, &pool_info, vulkan_allocation_callbacks,
  ...
  gpu_time_queries_manager->init(thread_frame_pools.data, allocator,
                                 creation.gpu_time_queries_per_frame, creation.num_threads,
                                 k_max_frames);
  ...
  for (u32 i = 0; i < thread_frame_pools.size; ++i) {
    ...
    vkCreateCommandPool(vulkan_device, &cmd_pool_info, vulkan_allocation_callbacks,
                        &pool.vulkan_command_pool);

    // Create timestamp query pool used for GPU timings.
    VkQueryPoolCreateInfo timestamp_pool_info{
        VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO, nullptr, 0, VK_QUERY_TYPE_TIMESTAMP,
        creation.gpu_time_queries_per_frame * 2u, 0};
    vkCreateQueryPool(vulkan_device, &timestamp_pool_info, vulkan_allocation_callbacks,
    ...
    VkQueryPoolCreateInfo statistics_pool_info{
        ...
        VK_QUERY_TYPE_PIPELINE_STATISTICS,        7,       0};
    statistics_pool_info.pipelineStatistics =
    ...
    vkCreateQueryPool(vulkan_device, &statistics_pool_info, vulkan_allocation_callbacks,
  ...
  descriptor_set_layouts.init(allocator, resource_pool_creation.descriptor_set_layouts,
  ...
  vkCreateSemaphore(vulkan_device, &semaphore_info, vulkan_allocation_callbacks,
                    &vulkan_image_acquired_semaphore);
  vkCreateSemaphore(vulkan_device, &semaphore_info, vulkan_allocation_callbacks,
  ...
  for (size_t i = 0; i < k_max_frames; i++) {
    vkCreateSemaphore(vulkan_device, &semaphore_info, vulkan_allocation_callbacks,
                      &vulkan_render_complete_semaphore[i]);

    if (!timeline_semaphore_extension_present) {
      vkCreateFence(vulkan_device, &fenceInfo, vulkan_allocation_callbacks,
                    &vulkan_command_buffer_executed_fence[i]);
    }
  }
  ...
  CommandBuffer** correctly_allocated_buffer =
      (CommandBuffer**)(memory + sizeof(GPUTimeQueriesManager));
  RASSERTM(queued_command_buffers == correctly_allocated_buffer,
           "Wrong calculations for queued command buffers arrays. Should be %p, but it is %p.",
           correctly_allocated_buffer, queued_command_buffers);
  ...
  BufferCreation fullscreen_vb_creation = {VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
  ...
  dummy_texture_creation.set_size(1, 1, 1)
      .set_flags(TextureFlags::Mask::RenderTarget_mask | TextureFlags::Mask::Compute_mask)
      .set_format_type(VK_FORMAT_R8_UINT, TextureType::Texture2D)
      .set_name("Dummy_texture");
  dummy_texture = create_texture(dummy_texture_creation);

  BufferCreation dummy_constant_buffer_creation = {VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                   ResourceUsageType::Immutable,
                                                   16,
                                                   0,
                                                   0,
  ...
  if (bindless_supported) {
    ...
    bindless_layout_creation.reset()
        .add_binding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, k_bindless_texture_binding,
                     k_max_bindless_resources, "BindlessTextures")
        .add_binding(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, k_bindless_texture_binding + 1,
                     k_max_bindless_resources, "BindlessImages")
        .set_set_index(0)
    ...
    bindless_set_creation.reset().set_layout(
        bindless_descriptor_set_layout);  // .texture( dummy_texture, 0 ).texture( dummy_texture, 1
                                          // );
  ...
  BufferCreation bc;
  bc.set(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT |
             VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
         ResourceUsageType::Immutable, dynamic_per_frame_size * k_max_frames)
...
void GpuDevice::shutdown() {
  ...
  for (size_t i = 0; i < k_max_frames; i++) {
    vkDestroySemaphore(vulkan_device, vulkan_render_complete_semaphore[i],
                       vulkan_allocation_callbacks);

    if (!timeline_semaphore_extension_present) {
      vkDestroyFence(vulkan_device, vulkan_command_buffer_executed_fence[i],
                     vulkan_allocation_callbacks);
    }
  }
  ...
  for (u32 i = 0; i < resource_deletion_queue.size; i++) {
    ...
    if (resource_deletion.current_frame == -1) continue;
  ...
  auto vkDestroyDebugUtilsMessengerEXT = (PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(
      vulkan_instance, "vkDestroyDebugUtilsMessengerEXT");
  vkDestroyDebugUtilsMessengerEXT(vulkan_instance, vulkan_debug_utils_messenger,
                                  vulkan_allocation_callbacks);
#endif  // IMGUI_VULKAN_DEBUG_REPORT

  // [TAG: BINDLESS]
  if (bindless_supported) {
    vkDestroyDescriptorPool(vulkan_device, vulkan_bindless_descriptor_pool,
                            vulkan_allocation_callbacks);
  }
  ...
  for (u32 i = 0; i < thread_frame_pools.size; ++i) {
    GpuThreadFramePools& pool = thread_frame_pools[i];
    vkDestroyQueryPool(vulkan_device, pool.vulkan_timestamp_query_pool,
                       vulkan_allocation_callbacks);
    vkDestroyQueryPool(vulkan_device, pool.vulkan_pipeline_stats_query_pool,
                       vulkan_allocation_callbacks);
    vkDestroyCommandPool(vulkan_device, pool.vulkan_command_pool, vulkan_allocation_callbacks);
  }

  // Memory: this contains allocations for gpu timestamp memory, queued command buffers and render
  // frames.
...
// Resource Creation ////////////////////////////////////////////////////////////
static void vulkan_create_texture_view(GpuDevice& gpu, const TextureViewCreation& creation,
                                       Texture* texture) {
  ...
  if (TextureFormat::has_depth_or_stencil(texture->vk_format)) {
    info.subresourceRange.aspectMask =
        TextureFormat::has_depth(texture->vk_format) ? VK_IMAGE_ASPECT_DEPTH_BIT : 0;
    // TODO:gs
    // info.subresourceRange.aspectMask |= TextureFormat::has_stencil( creation.format ) ?
  ...
  check(vkCreateImageView(gpu.vulkan_device, &info, gpu.vulkan_allocation_callbacks,
...
static VkImageUsageFlags vulkan_get_image_usage(const TextureCreation& creation) {
  const bool is_render_target =
      (creation.flags & TextureFlags::RenderTarget_mask) == TextureFlags::RenderTarget_mask;
  const bool is_compute_used =
      (creation.flags & TextureFlags::Compute_mask) == TextureFlags::Compute_mask;
  const bool is_shading_rate_texture =
      (creation.flags & TextureFlags::ShadingRate_mask) == TextureFlags::ShadingRate_mask;
...
static void vulkan_create_texture(GpuDevice& gpu, const TextureCreation& creation,
                                  TextureHandle handle, Texture* texture) {
  bool is_cubemap = false;
  u32 layer_count = creation.array_layer_count;
  if (creation.type == TextureType::TextureCube ||
      creation.type == TextureType::Texture_Cube_Array) {
    is_cubemap = true;
  }

  const bool is_sparse_texture =
      (creation.flags & TextureFlags::Sparse_mask) == TextureFlags::Sparse_mask;
  ...
  texture->array_base_layer =
      0;  // For new textures, we have a view that is for all mips and layers.
  ...
  if (creation.alias.index == k_invalid_texture.index) {
    if (is_sparse_texture) {
      check(vkCreateImage(gpu.vulkan_device, &image_info, gpu.vulkan_allocation_callbacks,
                          &texture->vk_image));
    } else {
      check(vmaCreateImage(gpu.vma_allocator, &image_info, &memory_info, &texture->vk_image,
    ...
    check(vmaCreateAliasingImage(gpu.vma_allocator, alias_texture->vma_allocation, &image_info,
                                 &texture->vk_image));
  ...
  tvc.set_mips(0, creation.mip_level_count)
      .set_array(0, layer_count)
      .set_name(creation.name)
      .set_view_type(to_vk_image_view_type(creation.type));
...
static void upload_texture_data(Texture* texture, void* upload_data, GpuDevice& gpu) {
  ...
  (vmaCreateBuffer(gpu.vma_allocator, &buffer_info, &memory_info, &staging_buffer,
                   &staging_allocation, &allocation_info));
  ...
  util_add_image_barrier(&gpu, command_buffer->vk_command_buffer, texture->vk_image,
  ...
  util_add_image_barrier(
      &gpu, command_buffer->vk_command_buffer, texture->vk_image,
...
TextureHandle GpuDevice::create_texture(const TextureCreation& creation) {
  ...
  resource_tracker.track_create_resource(ResourceUpdateType::Texture, resource_index,
                                         creation.name);
...
void dump_shader_code(StringBuffer& temp_string_buffer, cstring code, VkShaderStageFlagBits stage,
                      cstring name) {
  rprint("Error in creation of shader %s, stage %s. Writing shader:\n", name,
         to_stage_defines(stage));
  ...
  while (current_code) {
    ...
    char* line =
        temp_string_buffer.append_use_substring(current_code, 0, (end_of_line - current_code));
...
VkShaderModuleCreateInfo GpuDevice::compile_shader(cstring code, u32 code_size,
                                                   VkShaderStageFlagBits stage, cstring name) {
  ...
  // Compile to SPV
#if defined(_MSC_VER)
  char* glsl_compiler_path =
      temp_string_buffer.append_use_f("%sglslangValidator.exe", vulkan_binaries_path);
  ...
  char* arguments = temp_string_buffer.append_use_f(
      "glslangValidator.exe %s -V --target-env vulkan1.2 -o %s -S %s --D %s --D %s", temp_filename,
      final_spirv_filename, to_compiler_extension(stage), stage_define, to_stage_defines(stage));
#else
  char* glsl_compiler_path =
      temp_string_buffer.append_use_f("%sglslangValidator", vulkan_binaries_path);
  char* final_spirv_filename = temp_string_buffer.append_use("shader_final.spv");
  char* arguments = temp_string_buffer.append_use_f(
      "%s -V --target-env vulkan1.2 -o %s -S %s --D %s --D %s", temp_filename, final_spirv_filename,
      to_compiler_extension(stage), stage_define, to_stage_defines(stage));
#endif
  ...
  if (optimize_shaders) {
    ...
    char* spirv_optimizer_path =
        temp_string_buffer.append_use_f("%sspirv-opt.exe", vulkan_binaries_path);
    char* optimized_spirv_filename = temp_string_buffer.append_use_f("shader_opt.spv");
    char* spirv_opt_arguments =
        temp_string_buffer.append_use_f("spirv-opt.exe -O --preserve-bindings %s -o %s",
                                        final_spirv_filename, optimized_spirv_filename);

    process_execute(".", spirv_optimizer_path, spirv_opt_arguments, "");

    // Read back SPV file.
    shader_create_info.pCode = reinterpret_cast<const u32*>(file_read_binary(
        optimized_spirv_filename, temporary_allocator, &shader_create_info.codeSize));

    file_delete(optimized_spirv_filename);
  } else {
    // Read back SPV file.
    shader_create_info.pCode = reinterpret_cast<const u32*>(
        file_read_binary(final_spirv_filename, temporary_allocator, &shader_create_info.codeSize));
  }
...
ShaderStateHandle GpuDevice::create_shader_state(const ShaderStateCreation& creation) {
  ...
  resource_tracker.track_create_resource(ResourceUpdateType::ShaderState, handle.index,
                                         creation.name);
  ...
  shader_state->parse_result =
      (spirv::ParseResult*)allocator->allocate(sizeof(spirv::ParseResult), 64);
  ...
  for (compiled_shaders = 0; compiled_shaders < creation.stages_count; ++compiled_shaders) {
    ...
    // Spir-V file is not generated when there is a compilation error, we can use this to know when
    // compilation is succeded.
    if (shader_create_info.pCode) {
      // Parse the generated Spir-V to obtain specialization constants informations.
      spirv::parse_binary(shader_create_info.pCode, shader_create_info.codeSize, name_buffer,
                          shader_state->parse_result);

      // Compile shader module
      VkPipelineShaderStageCreateInfo& shader_stage_info =
          shader_state->shader_stage_info[compiled_shaders];
      ...
      if (shader_state->parse_result->specialization_constants_count) {
        specialization_info.mapEntryCount =
            shader_state->parse_result->specialization_constants_count;
        // NOTE: we assume specialization constants to either be i32,u32 or floats.
        specialization_info.dataSize =
            shader_state->parse_result->specialization_constants_count * sizeof(u32);
        ...
        for (u32 i = 0; i < shader_state->parse_result->specialization_constants_count; ++i) {
          const spirv::SpecializationConstant& specialization_constant =
              shader_state->parse_result->specialization_constants[i];
    ...
    switch (stage.type) {
      case VK_SHADER_STAGE_RAYGEN_BIT_KHR: {
        VkRayTracingShaderGroupCreateInfoKHR& shader_group_info =
            shader_state->shader_group_info[compiled_shaders];
...
PipelineHandle GpuDevice::create_pipeline(const PipelineCreation& creation,
  ...
  if (cache_path != nullptr && cache_exists) {
    ...
    if (cache_header->deviceID == vulkan_physical_properties.deviceID &&
        ...
        memcmp(cache_header->pipelineCacheUUID, vulkan_physical_properties.pipelineCacheUUID,
               VK_UUID_SIZE) == 0) {
    ...
    check(vkCreatePipelineCache(vulkan_device, &pipeline_cache_create_info,
  ...
  for (u32 l = 0; l < num_active_layouts; ++l) {
    ...
    if (l == 0) {
      ...
      pipeline->descriptor_set_layout_handles[l] =
          create_descriptor_set_layout(shader_state_data->parse_result->sets[l]);
    }

    DescriptorSetLayout* descriptor_set_layout =
        access_descriptor_set_layout(pipeline->descriptor_set_layout_handles[l]);
  ...
  check(vkCreatePipelineLayout(vulkan_device, &pipeline_layout_info, vulkan_allocation_callbacks,
                               &pipeline_layout));
  ...
  if (shader_state_data->graphics_pipeline) {
    ...
    if (creation.vertex_input.num_vertex_attributes) {
      for (u32 i = 0; i < creation.vertex_input.num_vertex_attributes; ++i) {
        ...
        vertex_attributes[i] = {vertex_attribute.location, vertex_attribute.binding,
                                to_vk_vertex_format(vertex_attribute.format),
                                vertex_attribute.offset};
      }

      vertex_input_info.vertexAttributeDescriptionCount =
          creation.vertex_input.num_vertex_attributes;
    ...
    if (creation.blend_state.active_states) {
      RASSERTM(creation.blend_state.active_states == creation.render_pass.num_color_formats,
               "Blend states (count: %u) mismatch with output targets (count %u)!If blend states "
               "are active, they must be defined for all outputs",
               creation.blend_state.active_states, creation.render_pass.num_color_formats);
    ...
    color_blending.attachmentCount = creation.blend_state.active_states
                                         ? creation.blend_state.active_states
    ...
    check(vkCreateGraphicsPipelines(vulkan_device, pipeline_cache, 1, &pipeline_info,
    ...
    check(vkCreateRayTracingPipelinesKHR(vulkan_device, VK_NULL_HANDLE, pipeline_cache, 1,
                                         &pipeline_info, vulkan_allocation_callbacks,
    ...
    shader_binding_table_data.init(temporary_allocator, shader_binding_table_size,
                                   shader_binding_table_size);

    check(vkGetRayTracingShaderGroupHandlesKHR(
        vulkan_device, pipeline->vk_pipeline, 0, shader_state_data->active_shaders,
        shader_binding_table_size, shader_binding_table_data.data));

    BufferCreation shader_binding_table_creation{};
    shader_binding_table_creation
        .set(VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR |
    ...
    shader_binding_table_creation.set_data(shader_binding_table_data.data + group_handle_size)
        .set_name("shader_binding_table_hit");
    pipeline->shader_binding_table_hit = create_buffer(shader_binding_table_creation);

    shader_binding_table_creation.set_data(shader_binding_table_data.data + (group_handle_size * 2))
        .set_name("shader_binding_table_miss");
    ...
    check(vkCreateComputePipelines(vulkan_device, pipeline_cache, 1, &pipeline_info,
                                   vulkan_allocation_callbacks, &pipeline->vk_pipeline));
...
BufferHandle GpuDevice::create_buffer(const BufferCreation& creation) {
  ...
  static const VkBufferUsageFlags k_dynamic_buffer_mask = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT |
                                                          VK_BUFFER_USAGE_INDEX_BUFFER_BIT |
  ...
  VkBufferCreateInfo buffer_info{VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};
  buffer_info.usage =
      VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT | creation.type_flags;
  ...
  VmaAllocationInfo allocation_info{};
  check(vmaCreateBuffer(vma_allocator, &buffer_info, &allocation_create_info, &buffer->vk_buffer,
...
DescriptorSetLayoutHandle GpuDevice::create_descriptor_set_layout(
  ...
  resource_tracker.track_create_resource(ResourceUpdateType::DescriptorSetLayout, handle.index,
                                         creation.name);
  ...
  for (u32 r = 0; r < creation.num_bindings; ++r) {
    ...
    max_binding = hydra_max(max_binding, input_binding.index);
  }
  max_binding += 1;
  ...
  u8* memory = rallocam(
      ((sizeof(VkDescriptorSetLayoutBinding) + sizeof(DescriptorBinding)) * creation.num_bindings) +
          (sizeof(u8) * max_binding),
      allocator);
  descriptor_set_layout->bindings = (DescriptorBinding*)memory;
  descriptor_set_layout->vk_binding =
      (VkDescriptorSetLayoutBinding*)(memory + sizeof(DescriptorBinding) * creation.num_bindings);
  descriptor_set_layout->index_to_binding =
      (u8*)(descriptor_set_layout->vk_binding + creation.num_bindings);
  ...
  for (u32 r = 0; r < creation.num_bindings; ++r) {
    ...
    // [TAG: BINDLESS]
    // Skip bindings for images and textures as they are bindless, thus bound in the global bindless
    // arrays (one for images, one for textures).
    // TODO(marco): better solution to allow individual image views to be bound
    if (creation.set_index == 0 && skip_bindless_bindings &&
    ...
    vk_binding.descriptorType = vk_binding.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER
  ...
  if (creation.bindless) {
    ...
    vkCreateDescriptorSetLayout(vulkan_device, &layout_info, vulkan_allocation_callbacks,
...
//
//
void GpuDevice::fill_write_descriptor_sets(
    GpuDevice& gpu, const DescriptorSetLayout* descriptor_set_layout, DescriptorSet* descriptor_set,
    VkWriteDescriptorSet* descriptor_write, VkDescriptorBufferInfo* buffer_info,
    VkDescriptorImageInfo* image_info, VkSampler vk_default_sampler, u32& num_resources) {
  ...
  for (u32 r = 0; r < num_resources; r++) {
    ...
    if (binding_data_index >= descriptor_set_layout->num_bindings) {
      rprint("Error adding binding %u, layout has max %u bindings\n", binding_data_index,
             descriptor_set_layout->num_bindings);
      continue;
    }

    // [TAG: BINDLESS]
    // Skip bindless descriptors as they are bound in the global bindless arrays.
    if (descriptor_set_layout->set_index == 0 && skip_bindless_bindings &&
        (binding.type == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER ||
         binding.type == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE)) {
      continue;
    }
    ...
    switch (binding.type) {
      ...
      case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER: {
        ...
        descriptor_write[i].descriptorType = buffer->usage == ResourceUsageType::Dynamic
                                                 ? VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC
...
DescriptorSetHandle GpuDevice::create_descriptor_set(const DescriptorSetCreation& creation) {
  ...
  resource_tracker.track_create_resource(ResourceUpdateType::DescriptorSet, handle.index,
                                         creation.name);
  ...
  alloc_info.descriptorPool =
      descriptor_set_layout->bindless ? vulkan_bindless_descriptor_pool : vulkan_descriptor_pool;
  ...
  // Cache data
  u8* memory = rallocam(
      (sizeof(ResourceHandle) + sizeof(SamplerHandle) + sizeof(u16)) * creation.num_resources,
      allocator);
  descriptor_set->resources = (ResourceHandle*)memory;
  descriptor_set->samplers =
      (SamplerHandle*)(memory + sizeof(ResourceHandle) * creation.num_resources);
  descriptor_set->bindings =
      (u16*)(memory + (sizeof(ResourceHandle) + sizeof(SamplerHandle)) * creation.num_resources);
  descriptor_set->num_resources = creation.num_resources;
  descriptor_set->layout = descriptor_set_layout;

  RASSERTM(creation.num_resources < k_max_descriptors_per_set,
           "Overflow in resources, please bump k_max_descriptors_per_set.");

  // TODO: fix gltf problems and enable this. It asserts when creating draws.
  if (descriptor_set_layout->set_index != 0) {
    // RASSERTM( creation.num_resources == descriptor_set_layout->num_bindings, "DescriptorSet
    // creation mismatch: passed descriptors %u, layout descriptors %u\n", creation.num_resources,
    // descriptor_set_layout->num_bindings );
  }
  ...
  qsort(sorting_data, creation.num_resources, sizeof(DescriptorSortingData),
        sorting_descriptor_func);
  for (u32 r = 0; r < creation.num_resources; r++) {
  ...
  fill_write_descriptor_sets(*this, descriptor_set_layout, descriptor_set, descriptor_write,
                             buffer_info, image_info, vk_default_sampler->vk_sampler,
                             num_resources);
...
static void vulkan_create_framebuffer(GpuDevice& gpu, Framebuffer* framebuffer) {
  ...
  check(vkCreateFramebuffer(gpu.vulkan_device, &framebuffer_info, nullptr,
                            &framebuffer->vk_framebuffer));
  gpu.set_resource_name(VK_OBJECT_TYPE_FRAMEBUFFER, (u64)framebuffer->vk_framebuffer,
                        framebuffer->name);
}

//
//
static VkRenderPass vulkan_create_render_pass(GpuDevice& gpu, const RenderPassOutput& output,
                                              cstring name) {
  ...
  for (u32 active_attachments = 0; active_attachments < output.num_color_formats;
       ++active_attachments) {
...
//
//
static RenderPassOutput fill_render_pass_output(GpuDevice& gpu,
                                                const RenderPassCreation& creation) {
  RenderPassOutput output;
  output.reset();

  for (u32 i = 0; i < creation.num_render_targets; ++i) {
    output.color(creation.color_formats[i], creation.color_final_layouts[i],
                 creation.color_operations[i]);
  }
...
RenderPassHandle GpuDevice::create_render_pass(const RenderPassCreation& creation) {
  ...
  resource_tracker.track_create_resource(ResourceUpdateType::RenderPass, handle.index,
                                         creation.name);
  ...
  // render_pass->vk_render_pass = vulkan_create_render_pass( *this, render_pass->output,
  // creation.name );

  if (!dynamic_rendering_extension_present) {
    // TODO(marco): create new version that uses VkCreateRenderPass2 if available to support
...
FramebufferHandle GpuDevice::create_framebuffer(const FramebufferCreation& creation) {
  ...
  resource_tracker.track_create_resource(ResourceUpdateType::Framebuffer, handle.index,
                                         creation.name);
...
void GpuDevice::destroy_texture(TextureHandle texture) {
  if (texture.index < textures.pool_size) {
    ...
    // Do not add textures to deletion queue, textures will be deleted after bindless descriptor is
    // updated.
...
void GpuDevice::destroy_descriptor_set_layout(DescriptorSetLayoutHandle descriptor_set_layout) {
  if (descriptor_set_layout.index < descriptor_set_layouts.pool_size) {
    resource_tracker.track_destroy_resource(ResourceUpdateType::DescriptorSetLayout,
                                            descriptor_set_layout.index);

    resource_deletion_queue.push(
        {ResourceUpdateType::DescriptorSetLayout, descriptor_set_layout.index, current_frame, 1});
  } else {
...
void GpuDevice::destroy_descriptor_set(DescriptorSetHandle descriptor_set) {
  if (descriptor_set.index < descriptor_sets.pool_size) {
    resource_tracker.track_destroy_resource(ResourceUpdateType::DescriptorSet,
                                            descriptor_set.index);

    resource_deletion_queue.push(
        {ResourceUpdateType::DescriptorSet, descriptor_set.index, current_frame, 1});
  } else {
...
void GpuDevice::destroy_render_pass(RenderPassHandle render_pass) {
  if (render_pass.index < render_passes.pool_size) {
    ...
    resource_deletion_queue.push(
        {ResourceUpdateType::RenderPass, render_pass.index, current_frame, 1});
  } else {
...
void GpuDevice::destroy_framebuffer(FramebufferHandle framebuffer) {
  if (framebuffer.index < framebuffers.pool_size) {
    ...
    resource_deletion_queue.push(
        {ResourceUpdateType::Framebuffer, framebuffer.index, current_frame, 1});
  } else {
...
void GpuDevice::destroy_texture_instant(ResourceHandle texture) {
  ...
  if (v_texture) {
    ...
    if (v_texture->vma_allocation != 0 &&
        v_texture->parent_texture.index == k_invalid_texture.index) {
...
void GpuDevice::destroy_pipeline_instant(ResourceHandle pipeline) {
  ...
  if (v_pipeline) {
    ...
    vkDestroyPipelineLayout(vulkan_device, v_pipeline->vk_pipeline_layout,
                            vulkan_allocation_callbacks);
  }
...
void GpuDevice::destroy_descriptor_set_layout_instant(ResourceHandle descriptor_set_layout) {
  DescriptorSetLayout* v_descriptor_set_layout =
      (DescriptorSetLayout*)descriptor_set_layouts.access_resource(descriptor_set_layout);

  if (v_descriptor_set_layout) {
    vkDestroyDescriptorSetLayout(vulkan_device, v_descriptor_set_layout->vk_descriptor_set_layout,
                                 vulkan_allocation_callbacks);
...
void GpuDevice::destroy_render_pass_instant(ResourceHandle render_pass) {
  ...
  if (v_render_pass) {
    ...
    // vkDestroyRenderPass( vulkan_device, v_render_pass->vk_render_pass,
    // vulkan_allocation_callbacks );
  }
...
void GpuDevice::destroy_framebuffer_instant(ResourceHandle framebuffer) {
  ...
  if (v_framebuffer) {
    ...
    if (!dynamic_rendering_extension_present) {
      vkDestroyFramebuffer(vulkan_device, v_framebuffer->vk_framebuffer,
                           vulkan_allocation_callbacks);
    }
  }
...
void GpuDevice::destroy_shader_state_instant(ResourceHandle shader) {
  ...
  if (v_shader_state) {
    for (size_t i = 0; i < v_shader_state->active_shaders; i++) {
      vkDestroyShaderModule(vulkan_device, v_shader_state->shader_stage_info[i].module,
                            vulkan_allocation_callbacks);
    }
  }
...
void GpuDevice::create_swapchain() {
  ...
  // TODO: Windows only!
  VkBool32 surface_supported;
  vkGetPhysicalDeviceSurfaceSupportKHR(vulkan_physical_device, vulkan_main_queue_family,
                                       vulkan_window_surface, &surface_supported);
  if (surface_supported != VK_TRUE) {
    rprint("Error no WSI support on physical device 0\n");
  }

  VkSurfaceCapabilitiesKHR surface_capabilities;
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(vulkan_physical_device, vulkan_window_surface,
                                            &surface_capabilities);

  VkExtent2D swapchain_extent = surface_capabilities.currentExtent;
  if (swapchain_extent.width == UINT32_MAX) {
    swapchain_extent.width =
        clamp(swapchain_extent.width, surface_capabilities.minImageExtent.width,
  ...
  VkResult result =
      vkCreateSwapchainKHR(vulkan_device, &swapchain_create_info, 0, &vulkan_swapchain);
  check(result);
  ...
  if (swapchain_render_pass.index == k_invalid_index) {
    ...
    swapchain_pass_creation.add_attachment(
        vulkan_surface_format.format, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, RenderPassOperation::Clear);
    swapchain_pass_creation.set_depth_stencil_texture(
        VK_FORMAT_D32_SFLOAT, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
    swapchain_pass_creation.set_depth_stencil_operations(RenderPassOperation::Clear,
                                                         RenderPassOperation::Clear);
  ...
  vkGetSwapchainImagesKHR(vulkan_device, vulkan_swapchain, &vulkan_swapchain_image_count,
  ...
  for (u32 iv = 0; iv < vulkan_swapchain_image_count; iv++) {
    ...
    resource_tracker.track_create_resource(ResourceUpdateType::Texture,
                                           vk_framebuffer->color_attachments[0].index, "swapchain");
    ...
    depth_texture_creation.set_size(swapchain_width, swapchain_height, 1)
    ...
    util_add_image_barrier(this, command_buffer->vk_command_buffer, color->vk_image,
                           RESOURCE_STATE_UNDEFINED, RESOURCE_STATE_PRESENT, 0, 1, false);
  }
...
void GpuDevice::destroy_swapchain() {
  for (size_t iv = 0; iv < vulkan_swapchain_image_count; iv++) {
    ...
    for (u32 a = 0; a < vk_framebuffer->num_color_attachments; ++a) {
      ...
      resource_tracker.track_destroy_resource(ResourceUpdateType::Texture,
                                              vk_framebuffer->color_attachments[a].index);
    ...
    if (vk_framebuffer->depth_stencil_attachment.index != k_invalid_index) {
      resource_tracker.track_destroy_resource(ResourceUpdateType::Texture,
                                              vk_framebuffer->depth_stencil_attachment.index);
      destroy_texture_instant(vk_framebuffer->depth_stencil_attachment.index);
    }

    if (!dynamic_rendering_extension_present) {
      vkDestroyFramebuffer(vulkan_device, vk_framebuffer->vk_framebuffer,
                           vulkan_allocation_callbacks);
    }
...
void GpuDevice::update_descriptor_set_instant(const DescriptorSetUpdate& update) {
  ...
  DescriptorSet* dummy_delete_descriptor_set =
      access_descriptor_set(dummy_delete_descriptor_set_handle);
...
u32 GpuDevice::get_memory_heap_count() { return vma_allocator->GetMemoryHeapCount(); }
...
void GpuDevice::resize_texture_3d(TextureHandle texture, u32 width, u32 height, u32 depth) {
  ...
  TextureCreation tc;
  tc.set_flags(vk_texture->flags)
      .set_format_type(vk_texture->vk_format, vk_texture->type)
      .set_name(vk_texture->name)
...
PagePoolHandle GpuDevice::allocate_texture_pool(TextureHandle texture_handle, u32 pool_size) {
  ...
  vkGetPhysicalDeviceSparseImageFormatProperties2(vulkan_physical_device, &format_info,
  ...
  vkGetPhysicalDeviceSparseImageFormatProperties2(vulkan_physical_device, &format_info,
                                                  &property_count, properties.data);
  ...
  VkImageSparseMemoryRequirementsInfo2 sparse_memory_requirement_info{
  ...
  page_pool->block_size =
      memory_requirements
          .alignment;  // NOTE(marco): alignment corresponds to block size for sparse textures
  ...
  vmaAllocateMemoryPages(vma_allocator, &page_memory_requirements, &allocation_create_info,
                         block_count, page_pool->vma_allocations.data, nullptr);
...
void GpuDevice::destroy_page_pool(PagePoolHandle pool_handle) {
  if (pool_handle.index < page_pools.pool_size) {
    ...
    resource_deletion_queue.push(
        {ResourceUpdateType::PagePool, pool_handle.index, current_frame + k_max_frames, 1});
  } else {
...
void GpuDevice::destroy_page_pool_instant(ResourceHandle handle) {
  ...
  if (page_pool) {
    vmaFreeMemoryPages(vma_allocator, page_pool->vma_allocations.size,
                       page_pool->vma_allocations.data);
...
void GpuDevice::bind_texture_pages(PagePoolHandle pool_handle, TextureHandle texture_handle, u32 x,
                                   u32 y, u32 width, u32 height, u32 layer) {
  PagePool* page_pool = access_page_pool(pool_handle);
  if (page_pool == nullptr) {
    RASSERT(false);
    return;
  ...
  VkImageAspectFlags aspect = TextureFormat::has_depth(texture->vk_format)
...
void GpuDevice::fill_barrier(FramebufferHandle framebuffer, ExecutionBarrier& out_barrier) {
  ...
  if (vk_framebuffer) {
    const u32 rts = vk_framebuffer->num_color_attachments;
    for (u32 i = 0; i < rts; ++i) {
      out_barrier.image_barriers[out_barrier.num_image_barriers++].texture =
          vk_framebuffer->color_attachments[i];
    }

    if (vk_framebuffer->depth_stencil_attachment.index != k_invalid_index) {
      out_barrier.image_barriers[out_barrier.num_image_barriers++].texture =
          vk_framebuffer->depth_stencil_attachment;
    }
  }
}
...
void GpuDevice::new_frame() {
  ...
  dynamic_max_per_frame_size = hydra_max(used_size, dynamic_max_per_frame_size);
...
void GpuDevice::present(CommandBuffer* async_compute_command_buffer) {
  ...
  for (u32 c = 0; c < num_queued_command_buffers; c++) {
    ...
    if (command_buffer->thread_frame_pool->time_queries->allocated_time_query) {
      vkCmdEndQuery(command_buffer->vk_command_buffer,
                    command_buffer->thread_frame_pool->vulkan_pipeline_stats_query_pool, 0);
    }
  ...
  if (texture_to_update_bindless.size) {
    ...
    for (i32 it = texture_to_update_bindless.size - 1; it >= 0; it--) {
      ...
      // if ( texture_to_update.current_frame == current_frame )
      {
        ...
        // rprint( "%s texture %u\n", add_texture_to_delete ? "Deleting" : "Updating",
        ...
        if (add_texture_to_delete) {
          resource_deletion_queue.push(
              {ResourceUpdateType::Texture, texture->handle.index, current_frame, 1});
        }
        ...
        if (texture->flags & TextureFlags::Compute_mask) {
          ...
          VkDescriptorImageInfo& descriptor_image_info_compute =
    ...
    if (current_write_index) {
      vkUpdateDescriptorSets(vulkan_device, current_write_index, bindless_descriptor_writes, 0,
                             nullptr);
    }
  }
  ...
  if (has_pending_sparse_bindings) {
    ...
    sparse_binding_infos.init(allocator, pending_sparse_memory_info.size,
                              pending_sparse_memory_info.size);
  ...
  if (timeline_semaphore_extension_present) {
    ...
    if (synchronization2_extension_present) {
      ...
      wait_semaphores.push({VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO_KHR, nullptr,
                            vulkan_image_acquired_semaphore, 0,
                            VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT_KHR, 0});

      if (wait_for_compute_semaphore) {
        wait_semaphores.push({VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO_KHR, nullptr,
                              vulkan_compute_semaphore, last_compute_semaphore_value,
                              VK_PIPELINE_STAGE_2_VERTEX_ATTRIBUTE_INPUT_BIT_KHR, 0});
      }

      if (wait_for_timeline_semaphore) {
        wait_semaphores.push({VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO_KHR, nullptr,
                              vulkan_graphics_semaphore, absolute_frame - (k_max_frames - 1),
                              VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT_KHR, 0});
      }

      if (has_pending_sparse_bindings) {
        wait_semaphores.push({VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO_KHR, nullptr,
                              vulkan_bind_semaphore, 0, VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT_KHR,
                              0});
      }

      VkSemaphoreSubmitInfoKHR signal_semaphores[]{
          {VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO_KHR, nullptr, *render_complete_semaphore, 0,
           VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT_KHR, 0},
          {VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO_KHR, nullptr, vulkan_graphics_semaphore,
           absolute_frame + 1, VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT_KHR, 0}};
      ...
      // NOTE(marco): timeline semaphore values have to be monotonically increasing, so we need to
      // start from 1 NOTE(marco): we still have to provide a value even for non-timeline semaphores
  ...
  if (timestamps_enabled) {
    ...
    for (u32 i = 0; i < num_threads; ++i) {
      ...
      if (time_query && time_query->allocated_time_query) {
        ...
        vkGetQueryPoolResults(vulkan_device, thread_pool.vulkan_timestamp_query_pool, 0,
                              query_count * 2, sizeof(u64) * query_count * 2, timestamps_data,
        ...
        u64* pipeline_statistics_data =
            (u64*)ralloca(GpuPipelineStatistics::Count * sizeof(u64), temporary_allocator);
        vkGetQueryPoolResults(vulkan_device, thread_pool.vulkan_pipeline_stats_query_pool, 0, 1,
                              GpuPipelineStatistics::Count * sizeof(u64), pipeline_statistics_data,
                              sizeof(u64), VK_QUERY_RESULT_64_BIT);

        for (u32 i = 0; i < GpuPipelineStatistics::Count; ++i) {
          gpu_time_queries_manager->frame_pipeline_statistics.statistics[i] +=
              pipeline_statistics_data[i];
        }
      }
    ...
    // rprint( "%llu %f\n", gpu_time_queries_manager->frame_pipeline_statistics.statistics[ 6 ], (
    // gpu_time_queries_manager->frame_pipeline_statistics.statistics[ 6 ] * 1.0 ) / (
  ...
  if (timeline_semaphore_extension_present) {
    ...
    if (synchronization2_extension_present) {
      VkSemaphoreSubmitInfoKHR wait_semaphores[]{
          {VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO_KHR, nullptr, vulkan_compute_semaphore,
           last_compute_semaphore_value, VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT_KHR, 0}};
    ...
    if (synchronization2_extension_present) {
      ...
      VkSemaphoreSubmitInfoKHR signal_semaphores[]{
          {VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO_KHR, nullptr, vulkan_compute_semaphore, 0,
           VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT_KHR, 0},
      };
...
void GpuDevice::set_present_mode(PresentMode::Enum mode) {
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(vulkan_physical_device, vulkan_window_surface,
                                            &supported_count, NULL);
  RASSERT(supported_count < 8);
  vkGetPhysicalDeviceSurfacePresentModesKHR(vulkan_physical_device, vulkan_window_surface,
                                            &supported_count, supported_mode_allocated);
  ...
  vulkan_swapchain_image_count =
      3;  // vulkan_present_mode == VK_PRESENT_MODE_IMMEDIATE_KHR ? 2 : 3;
...
void GpuDevice::query_texture(TextureHandle texture, TextureDescription& out_description) {
  if (texture.index != k_invalid_index) {
    ...
    out_description.render_target =
        (texture_data->flags & TextureFlags::RenderTarget_mask) == TextureFlags::RenderTarget_mask;
    out_description.compute_access =
        (texture_data->flags & TextureFlags::Compute_mask) == TextureFlags::Compute_mask;
...
void GpuDevice::query_descriptor_set_layout(DescriptorSetLayoutHandle descriptor_set_layout,
                                            DescriptorSetLayoutDescription& out_description) {
  if (descriptor_set_layout.index != k_invalid_index) {
    const DescriptorSetLayout* descriptor_set_layout_data =
        access_descriptor_set_layout(descriptor_set_layout);
...
void GpuDevice::query_descriptor_set(DescriptorSetHandle descriptor_set,
                                     DesciptorSetDescription& out_description) {
...
void* GpuDevice::map_buffer(const MapBufferParameters& parameters) {
  if (parameters.buffer.index == k_invalid_index) return nullptr;
...
void GpuDevice::unmap_buffer(const MapBufferParameters& parameters) {
  if (parameters.buffer.index == k_invalid_index) return;

  Buffer* buffer = access_buffer(parameters.buffer);
  if (buffer->parent_buffer.index == dynamic_buffer.index) return;
...
void* GpuDevice::dynamic_allocate(u32 size) {
  ...
  dynamic_allocated_size += (u32)hydra::memory_align(size, ubo_alignment);
  return mapped_memory;
}

void GpuDevice::set_buffer_global_offset(BufferHandle buffer, u32 offset) {
  if (buffer.index == k_invalid_index) return;
...
BufferHandle GpuDevice::get_fullscreen_vertex_buffer() const { return fullscreen_vertex_buffer; }

RenderPassHandle GpuDevice::get_swapchain_pass() const { return swapchain_render_pass; }

FramebufferHandle GpuDevice::get_current_framebuffer() const {
  return vulkan_swapchain_framebuffers[vulkan_image_index];
}

TextureHandle GpuDevice::get_dummy_texture() const { return dummy_texture; }

BufferHandle GpuDevice::get_dummy_constant_buffer() const { return dummy_constant_buffer; }
...
DescriptorSetLayout* GpuDevice::access_descriptor_set_layout(
...
const DescriptorSetLayout* GpuDevice::access_descriptor_set_layout(
    DescriptorSetLayoutHandle descriptor_set_layout) const {
  return (const DescriptorSetLayout*)descriptor_set_layouts.access_resource(
      descriptor_set_layout.index);
}

DescriptorSetLayoutHandle GpuDevice::get_descriptor_set_layout(PipelineHandle pipeline_handle,
                                                               int layout_index) {
