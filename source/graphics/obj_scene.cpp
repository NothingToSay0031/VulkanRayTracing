...
void ObjScene::add_mesh(cstring filename, cstring path, StackAllocator* temp_allocator,
  ...
  const aiScene* assimp_scene = aiImportFile(
  ...
  for (u32 material_index = 0; material_index < assimp_scene->mNumMaterials; ++material_index) {
    ...
    PBRMaterial hydra_material{};

    aiString texture_file;

    if (aiGetMaterialString(material, AI_MATKEY_TEXTURE(aiTextureType_DIFFUSE, 0), &texture_file) ==
        AI_SUCCESS) {
      hydra_material.diffuse_texture_index =
          load_texture(texture_file.C_Str(), path, async_loader, temp_allocator);
    } else {
      hydra_material.diffuse_texture_index = k_invalid_scene_texture_index;
    }

    if (aiGetMaterialString(material, AI_MATKEY_TEXTURE(aiTextureType_NORMALS, 0), &texture_file) ==
        AI_SUCCESS) {
      hydra_material.normal_texture_index =
          load_texture(texture_file.C_Str(), path, async_loader, temp_allocator);
    } else {
      hydra_material.normal_texture_index = k_invalid_scene_texture_index;
    }

    hydra_material.roughness_texture_index = k_invalid_scene_texture_index;
    hydra_material.occlusion_texture_index = k_invalid_scene_texture_index;

    aiColor4D color;
    if (aiGetMaterialColor(material, AI_MATKEY_COLOR_DIFFUSE, &color) == AI_SUCCESS) {
      hydra_material.base_color_factor = {color.r, color.g, color.b, 1.0f};
    } else {
      hydra_material.base_color_factor = {1.0f, 1.0f, 1.0f, 1.0f};
    }
    ...
    if (aiGetMaterialFloat(material, AI_MATKEY_SHININESS, &f_value) == AI_SUCCESS) {
      ...
      hydra_material.occlusion = hydra::max(powf((1.f - specular_exp), 2.f), 0.0001f);
    }

    if (aiGetMaterialFloat(material, AI_MATKEY_OPACITY, &f_value) == AI_SUCCESS) {
      hydra_material.base_color_factor.w = f_value;
    }

    materials.push(hydra_material);
  }
  ...
  for (u32 mesh_index = 0; mesh_index < assimp_scene->mNumMeshes; ++mesh_index) {
    ...
    for (u32 vertex_index = 0; vertex_index < mesh->mNumVertices; ++vertex_index) {
      ...
      vec3s normal = vec3s{mesh->mNormals[vertex_index].x, mesh->mNormals[vertex_index].y,
      ...
      tangents.push(vec3s{mesh->mTangents[vertex_index].x, mesh->mTangents[vertex_index].y,
    ...
    if (k_enable_physics) {
      BufferCreation creation{};
      sizet buffer_size =
          positions.size * sizeof(PhysicsVertexGpuData) + sizeof(PhysicsMeshGpuData);
      creation.set(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, ResourceUsageType::Immutable, buffer_size)
          .set_data(nullptr)
          .set_name("physics_mesh_data_cpu")
          .set_persistent(true);
      ...
      PhysicsVertexGpuData* vertex_data =
          (PhysicsVertexGpuData*)(physics_vertex_buffer->mapped_data + sizeof(PhysicsMeshGpuData));

      Array<VkDrawIndirectCommand> indirect_commands;
      indirect_commands.init(resident_allocator, physics_mesh->vertices.size,
                             physics_mesh->vertices.size);
      ...
      creation.reset()
          .set(VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
               ResourceUsageType::Immutable, buffer_size)
          .set_device_only(true)
          .set_name("physics_mesh_data_gpu");
      ...
      creation.reset()
          .set(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, ResourceUsageType::Immutable, buffer_size)
          .set_data(indirect_commands.data)
          .set_name("indirect_buffer_cpu");

      cpu_buffer = renderer->gpu->create_buffer(creation);

      creation.reset()
          .set(VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT,
               ResourceUsageType::Immutable, buffer_size)
          .set_device_only(true)
          .set_name("indirect_buffer_gpu");
  ...
  // Positions
  {
    ...
    creation.set(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, ResourceUsageType::Immutable, buffer_size)
        ...
        .set_name("obj_positions")
        .set_persistent(true);

    BufferHandle cpu_buffer = renderer->gpu->create_buffer(creation);

    creation.reset()
        .set(flags, ResourceUsageType::Immutable, buffer_size)
        .set_device_only(true)
  ...
  // Tangents
  {
    ...
    creation.set(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, ResourceUsageType::Immutable, buffer_size)
        .set_data(tangents.data)
        .set_name("obj_tangents")
        .set_persistent(true);
    ...
    creation.reset()
        .set(flags, ResourceUsageType::Immutable, buffer_size)
        .set_device_only(true)
        .set_name("tangent_attribute_buffer");
  ...
  // Normals
  {
    ...
    creation.set(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, ResourceUsageType::Immutable, buffer_size)
        .set_data(normals.data)
        .set_name("obj_normals")
        .set_persistent(true);
    ...
    creation.reset()
        .set(flags, ResourceUsageType::Immutable, buffer_size)
        .set_device_only(true)
        .set_name("normal_attribute_buffer");
  ...
  // TexCoords
  {
    ...
    creation.set(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, ResourceUsageType::Immutable, buffer_size)
        .set_data(uv_coords.data)
        .set_name("obj_tex_coords");
    ...
    creation.reset()
        .set(flags, ResourceUsageType::Immutable, buffer_size)
        .set_device_only(true)
        .set_name("texcoords_attribute_buffer");
  ...
  rprint(
      ...
      time_delta_seconds(end_creating_textures, end_reading_buffers_data),
      time_delta_seconds(end_reading_buffers_data, end_creating_buffers));
}

u32 ObjScene::load_texture(cstring texture_path, cstring path, AsynchronousLoader* async_loader,
...
void ObjScene::prepare_draws(Renderer* renderer, StackAllocator* scratch_allocator,
  ...
  buffer_creation.reset()
      .set(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, ResourceUsageType::Dynamic, sizeof(PhysicsSceneData))
      .set_name("physics_cb");
  ...
  for (u32 mesh_index = 0; mesh_index < meshes.size; ++mesh_index) {
    ...
    DescriptorSetLayoutHandle main_layout = renderer->gpu->get_descriptor_set_layout(
        mesh.pbr_material.material->technique->passes[pass_index].pipeline,
        k_material_descriptor_set_index);
    ds_creation.reset()
        .buffer(scene_cb, 0)
        .buffer(mesh.pbr_material.material_buffer, 2)
    ...
    if (mesh.physics_mesh != nullptr) {
      DescriptorSetLayoutHandle physics_layout = renderer->gpu->get_descriptor_set_layout(
          cloth_technique->passes[0].pipeline, k_material_descriptor_set_index);
      ds_creation.reset()
          .buffer(physics_cb, 0)
      ...
      DescriptorSetLayoutHandle debug_mesh_layout = renderer->gpu->get_descriptor_set_layout(
          debug_technique->passes[0].pipeline, k_material_descriptor_set_index);
      ds_creation.reset()
          .buffer(scene_cb, 0)
          .buffer(mesh.physics_mesh->gpu_buffer, 1)
          .set_layout(debug_mesh_layout);

      mesh.physics_mesh->debug_mesh_descriptor_set =
          renderer->gpu->create_descriptor_set(ds_creation);
    }
  }
