...
static void add_binding_if_unique(DescriptorSetLayoutCreation& creation,
...
void parse_binary(const u32* data, size_t data_size, StringBuffer& name_buffer,
    ...
    switch (id.op) {
      ...
      case (SpvOpSpecConstantComposite): {
        ...
        SpecializationConstant& specialization_constant =
            parse_result->specialization_constants[parse_result->specialization_constants_count];
        ...
        SpecializationName& specialization_name =
            parse_result->specialization_names[parse_result->specialization_constants_count];
        hydra::StringView::copy_to(id_spec_binding.name, specialization_name.name, 32);
    ...
    if (id.op == SpvOpVariable) {
      switch (id.storage_class) {
        ...
        case (SpvStorageClassUniformConstant): {
          if (id.set == k_bindless_set_index && (id.binding == k_bindless_texture_binding ||
  ...
  for (size_t i = 0; i < parse_result->set_count; i++) {
    ...
    qsort(layout_creation.bindings, layout_creation.num_bindings,
          sizeof(DescriptorSetLayoutCreation::Binding), sorting_func);
  }
}
