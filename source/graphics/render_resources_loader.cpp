...
// Utility methods ////////////////////////////////////////////////////////
static u64 shader_concatenate(cstring filename, hydra::StringBuffer& path_buffer,
                              hydra::StringBuffer& shader_buffer,
                              hydra::Allocator* temp_allocator);
static VkBlendFactor get_blend_factor(const std::string factor);
static VkBlendOp get_blend_op(const std::string op);
static bool parse_gpu_pipeline(nlohmann::json& pipeline, hydra::PipelineCreation& pc,
                               hydra::StringBuffer& path_buffer,
                               hydra::StringBuffer& shader_buffer,
                               hydra::Allocator* temp_allocator, hydra::Renderer* renderer,
                               hydra::FrameGraph* frame_graph,
                               hydra::StringBuffer& pass_name_buffer,
                               const Array<VertexInputCreation>& vertex_input_creations,
                               FlatHashMap<u64, u16>& name_to_vertex_inputs, cstring technique_name,
                               bool use_cache, bool parent_technique, bool& is_shader_changed);

// RenderResourcesLoader //////////////////////////////////////////////////
void RenderResourcesLoader::init(hydra::Renderer* renderer_,
                                 hydra::StackAllocator* temp_allocator_,
                                 hydra::FrameGraph* frame_graph_) {
  renderer = renderer_;
...
void RenderResourcesLoader::parse_gpu_technique(GpuTechniqueCreation& technique_creation,
                                                cstring json_path, bool use_shader_cache,
                                                bool& is_techinque_changed) {
  ...
  if (pipelines.is_array()) {
    ...
    for (u32 i = 0; i < size; ++i) {
      ...
      if (inherit_from.is_string()) {
        ...
        for (u32 ii = 0; ii < size; ++ii) {
          ...
          if (name == inherited_name) {
            add_pass = parse_gpu_pipeline(
                pipeline_i, pc, path_buffer, shader_code_buffer, temp_allocator, renderer,
                frame_graph, pass_name_buffer, vertex_input_creations, name_to_vertex_inputs,
                technique_creation.name, false, true, parent_shader_changed);
            break;
          }
        }
      }
      ...
      add_pass = add_pass &&
                 parse_gpu_pipeline(pipeline, pc, path_buffer, shader_code_buffer, temp_allocator,
                                    renderer, frame_graph, pass_name_buffer, vertex_input_creations,
                                    name_to_vertex_inputs, technique_creation.name,
                                    use_shader_cache, false, current_shader_changed);
...
GpuTechnique* RenderResourcesLoader::load_gpu_technique(cstring json_path, bool use_shader_cache,
  ...
  rprint("Created technique %s in %f seconds\n", technique_creation.name,
         time_from_seconds(begin_time));

  return technique;
}

void RenderResourcesLoader::reload_gpu_technique(cstring json_path, bool use_shader_cache,
                                                 bool& is_technique_changed) {
  i64 begin_time = time_now();
  sizet allocated_marker = temp_allocator->get_marker();
  ...
  if (is_technique_changed) {
    ...
    GpuTechnique* old_technique =
        renderer->resource_cache.techniques.get(hash_calculate(technique_creation.name));
...
u64 shader_concatenate(cstring filename, hydra::StringBuffer& path_buffer,
                       hydra::StringBuffer& shader_buffer, hydra::Allocator* temp_allocator) {
  using namespace hydra;

  u64 hashed_memory = 0;
  // Read file and concatenate it
  path_buffer.clear();
  cstring shader_path = path_buffer.append_use_f("%s%s", HYDRA_SHADER_FOLDER, filename);
  ...
  if (shader_read_result.data) {
    ...
    // Using strlne because file can contain impurities after the end, causing different hashes when
    // using shader_read_result.size.
    hashed_memory = hydra::hash_bytes(shader_read_result.data, strlen(shader_read_result.data));
  } else {
...
bool parse_gpu_pipeline(nlohmann::json& pipeline, hydra::PipelineCreation& pc,
                        hydra::StringBuffer& path_buffer, hydra::StringBuffer& shader_buffer,
                        hydra::Allocator* temp_allocator, hydra::Renderer* renderer,
                        hydra::FrameGraph* frame_graph, hydra::StringBuffer& pass_name_buffer,
                        const Array<VertexInputCreation>& vertex_input_creations,
                        FlatHashMap<u64, u16>& name_to_vertex_inputs, cstring technique_name,
                        bool use_cache, bool parent_technique, bool& shader_changed) {
  using json = nlohmann::json;
  using namespace hydra;
  ...
  if (!shaders.is_null()) {
    for (sizet s = 0; s < shaders.size(); ++s) {
      ...
      if (use_cache) {
        ...
        path_buffer.clear();
        shader_spirv_path = path_buffer.append_use_f(
            "%s/%s_%s_%s.spv", renderer->resource_cache.binary_data_folder, technique_name,
            pc.shaders.name, to_compiler_extension(shader_stage.type));
        shader_hash_path = path_buffer.append_use_f(
            "%s/%s_%s_%s.hash.cache", renderer->resource_cache.binary_data_folder, technique_name,
            pc.shaders.name, to_compiler_extension(shader_stage.type));
        // FileReadResult shader_read_result = file_read_text( shader_path, temp_allocator );
      ...
      if (compile_shader) {
        VkShaderModuleCreateInfo shader_create_info =
            renderer->gpu->compile_shader(code, code_size, shader_stage.type, pc.shaders.name);
        if (shader_create_info.pCode) {
          ...
          if (use_cache) {
            // Write hashes
            file_write_binary(shader_hash_path, &shader_file_hashes[0],
                              sizeof(u64) * shader_file_hashes_count);
            ...
            rprint( "\n\n" );*/
            // Write spirv
            file_write_binary(shader_spirv_path, (void*)shader_create_info.pCode,
                              sizeof(u32) * shader_create_info.codeSize);
          }

          // Only when compiling a shader we can say that it is changed.
          shader_changed = true;
        } else {
          rprint("Error compiling shader %s stage %s", pc.shaders.name,
  ...
  if (!blend_states.is_null()) {
    for (sizet b = 0; b < blend_states.size(); ++b) {
      ...
      blend_state.set_color(get_blend_factor(src_colour), get_blend_factor(dst_colour),
                            get_blend_op(blend_op));
    }
  }
  ...
  if (render_pass.is_string()) {
    ...
    if (node) {
      ...
      if (name == "swapchain") {
        ...
        if (render_pass) pc.render_pass = render_pass->output;
      }
    } else {
